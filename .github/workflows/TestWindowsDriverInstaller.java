...
public class TestWindowsDriverInstaller {
    ...
    public static void main(final String[] args) throws Exception
    {
        ...
        switch(backend) {
            ...
            case "drivers": {
                    final String                 driverBackend = System.getProperty(
                        "wdi.driverbackend", ""
                    );
                    ...
                    if( "PS1".equals(driverBackend) ) wdi = new WindowsDriverInstaller_PS1();
                        ...
                        "drivers mode requires -Dwdi.driverbackend=PS1|FFM, got: " + driverBackend
                    );
            ...
            case "auto": {
                    ...
                    System.out.println(
                        "create() returned: " + ( picked == null ? "null" : picked.getClass().getSimpleName() )
                    );
    ...
    )
    {
        ...
        try {
            ...
            printPair(
                label + ".isProviderAlreadyTrusted", wdi.isProviderAlreadyTrusted(provider)
            );

            if(runMutating) {
                printPair(
                    label + ".createAndTrustProvider", wdi.createAndTrustProvider(provider)
                );
                printPair(
                    label + ".isProviderAlreadyTrusted (after trust)",
                    wdi.isProviderAlreadyTrusted(provider)
                );
                ...
                java.nio.file.Files.writeString(
                    inf, WindowsDriverInstaller.generateWinUSBInf("1234", "5678", catName)
                );

                printPair(
                    label + ".createAndSignCatalog",
                    wdi.createAndSignCatalog( inf.toString(), provider )
                );
                printPair( label + ".installDriver", wdi.installDriver( inf.toString() ) );
            } // if
            else {
                System.out.println(
                    label + ": skipping mutating/elevated calls (run_mutating=false)"
                );
            }
    ...
    private static int testDriverKindsOn(
        final String                 label,
        final WindowsDriverInstaller wdi,
        final boolean                runMutating
    )
    {
        ...
        if(!runMutating) {
            System.out.println(
                "skipping (run_mutating=false) - installLibusbKInf()/installLibusb0Inf() are always mutating (trust+sign+install)"
            );
            return 0;
        }
    ...
    ) throws Exception
    {
        ...
        final java.nio.file.Path certSrc = java.nio.file.Paths.get(
            System.getProperty("java.io.tmpdir"), provider + ".cer"
        );
        copyIfExists( certSrc, outDir.resolve(prefix + "-cert.cer") );

        final java.nio.file.Path dir     = java.nio.file.Files.createTempDirectory(
            "wdi_cmp_" + prefix
        );
        ...
        printPair(
            prefix + ".createAndSignCatalog", wdi.createAndSignCatalog( inf.toString(), provider )
        );
    ...
    private static void copyIfExists(
        final java.nio.file.Path src, final java.nio.file.Path dst
