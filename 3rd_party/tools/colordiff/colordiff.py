...
def check_for_file_arguments(args):
    ...
    for arg in args:
        ...
        if ddash or arg == '-':
            nonopts += 1
            continue
        if not arg.startswith('-'): nonopts += 1
        if arg in ('--help', '--version', '-v'): nonopts += 1

    return nonopts


def detect_diff_type(record, allow_diffy):
    if re.match(r'^(\+\+\+ |--- |@@ )', record): return 'diffu'
    if re.match(r'^\*\*\*', record): return 'diffc'
    if re.match(r'^[0-9,]+[acd][0-9,]+$', record.rstrip('\n')): return 'diff'
    if allow_diffy and re.search(r'(\s\|\s|\s<$|\s>\s)', record): return 'diffy'
    if re.search(r'\[-.+?-\]', record, re.DOTALL) or re.search(r'\{\+.+?\+\}', record, re.DOTALL):
        return 'wdiff'
    if re.match(r'^Control files: lines which differ', record): return 'debdiff'
...
def parse_color_value(value, setting, config_file):
    ...
    if re.match(r'^[0-9]+$', value):
        n = int(value)
        if 0 <= n <= 255:
            if n < 8: return f'\033[0;3{n}m'

            elif n < 16: return f'\033[0;9{n - 8}m'
        ...
        try:
            with open(config_file, 'r', errors='replace') as f:
                for line in f:
                    ...
                    if setting == 'banner':
                        if value == 'yes': show_banner = True
                        elif value == 'no': show_banner = False
                        continue
                    ...
                    if setting == 'plain': plain_text = colourval
                    elif setting == 'oldtext': file_old = colourval
                    elif setting == 'newtext': file_new = colourval
                    elif setting == 'diffstuff': diff_stuff = colourval
                    elif setting == 'difffile': diff_file = colourval
                    elif setting == 'cvsstuff': cvs_stuff = colourval
                    else: print(f'Unknown option in {config_file}: {setting}', file=sys.stderr)
        except OSError:
            pass
...
def parse_args(argv):
    ...
    while i < len(argv):
        arg = argv[i]
        if arg == '--verifymode': verify_mode = True
        elif arg == '--fakeexitcode': fake_exitcode = True
        elif arg.startswith('--difftype='): specified_type = arg[len('--difftype='):]
        elif arg == '--difftype' and i + 1 < len(argv):
            i              += 1
            specified_type  = argv[i]
        elif arg.startswith('--color='): cmd_color_mode = arg[len('--color='):]
        elif arg == '--color' and i + 1 < len(argv):
            i              += 1
            cmd_color_mode  = argv[i]
        elif arg.startswith('--color-patches='): cmd_color_patch = arg[len('--color-patches='):]
        ...
        elif arg == '--color-term-output-only': color_term_only = True
        elif arg == '--banner': cmd_banner = True
        elif arg == '--no-banner': cmd_banner = False
        elif arg == '--help':
            show_help = True
...
def main():
    ...
    elif HOME: config_files.append(os.path.join(HOME, '.config', 'colordiff', 'colordiffrc'))
    if HOME:
    ...
    if cmd_color_mode in ('yes', 'always', 'auto'): color_mode = True
    elif cmd_color_mode in ('no', 'never'):
        color_mode = False

    # Resolve color_patch
    color_patch = cfg_color_patch
    if cmd_color_patch in ('yes', 'always', 'auto'): color_patch = True
    ...
    if cmd_banner is True: show_banner = True
    ...
    if diff_type == 'diffy':
        ...
        while checkbuffer:
            ...
            for i in range(len(expanded) - 2):
                if separator_col.get(i, 0) == 0: continue
                if re.match(r'^(Index: |={4,}|RCS file: |retrieving |diff )', expanded): continue
                subsub = expanded[i:i+2]
                if not re.match(r' [ (|<>]', subsub):
                    separator_col[i] = 0
                    if candidate_col.get(i, 0) > 0: possible_cols -= 1
    ...
    def next_line():
        if inputstream: return inputstream.pop(0)
        if lastline is not None: return input_iter.readline() or None
    ...
    def stream():
        ...
        while inputstream: yield inputstream.pop(0)
    ...
    for line in stream():
        ...
        if diff_type == 'diff':
            if line.startswith('<'): prefix = file_old
            elif line.startswith('>'): prefix = file_new
            elif line and line[0].isdigit(): prefix = diff_stuff
            elif re.match(r'^(Index: |={4,}|RCS file: |retrieving |diff )', line):
                prefix = cvs_stuff
            elif line.startswith('Only in'): prefix = diff_file
            else:
                prefix = plain_text
            ...
            elif re.match(r'^\*{4,}', line): prefix = diff_file
            elif line.startswith('Only in'): prefix = diff_file
            elif re.match(r'^\*\*\* [0-9]+,[0-9]+', line):
                prefix          = diff_file
                inside_file_old = True
            elif line.startswith('*** '): prefix = diff_file
            elif re.match(r'^--- [0-9]+,[0-9]+', line):
                prefix          = diff_file
                inside_file_old = False
            elif line.startswith('--- '): prefix = diff_file
            elif line.startswith('!'): prefix = file_old if inside_file_old else file_new
            elif re.match(r'^(Index: |={4,}|RCS file: |retrieving |diff )', line):
                prefix = cvs_stuff
            else:
                prefix = plain_text
            ...
            if re.match(r'^(---|\+\+\+) ', line): prefix = diff_file
                ...
                if '-' in diff_marks: prefix = file_old
                elif '+' in diff_marks: prefix = file_new
            elif line.startswith('Only in'): prefix = diff_file
            ...
            if len(expanded) > diffy_sep_col + 2:
                ...
                if sepchars == ' <': prefix = file_old
                elif sepchars == ' |': prefix = diff_stuff
                elif sepchars == ' >': prefix = file_new
                else: prefix = plain_text
            elif line.startswith('Only in'): prefix = diff_file
            else: prefix = plain_text
            line = expanded
