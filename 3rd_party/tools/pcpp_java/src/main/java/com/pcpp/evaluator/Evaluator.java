...
public class Evaluator {
    ...
    private static final Set<String> SKIP_TYPES = new HashSet<>( Arrays.asList(
        CppTokens.CPP_WS, CppTokens.CPP_LINECONT, CppTokens.CPP_COMMENT1, CppTokens.CPP_COMMENT2
    ) );
    ...
    private static class Parser {
        ...
        private Value parsePrimary()
        {
            ...
            if( CppTokens.CPP_STRING.equals(t.type) ) {
                ...
                return new Value(
                    0, new SyntaxException("String literal in #if expression: " + t.value)
                );
            } // if
            ...
            if( CppTokens.CPP_ID.equals(t.type) ) {
                ...
                if( check(CppTokens.CPP_LPAREN) ) {
                    ...
                    if( !check(
                        ...
                        "Missing ')' in function call to " + name
                    );
                    consume(); // Eat ')'
                    Function<Value, Value> fn = functions.get(name);
                    if(fn == null) return new Value(
                        0, new SyntaxException("Unknown function " + name)
                    );
                    try {
                        return fn.apply(arg);
                    }
                ...
                if( val == null && !identifiers.containsKey(
                    name
                ) ) return new Value(
                    0, new SyntaxException("Unknown identifier " + name)
                );
                ...
                return new Value(
                    0, new SyntaxException("Unknown identifier value type for " + name)
                );
            } // if
        ...
        private int infixPrec(LexToken t)
        {
            switch(t.type) {
                ...
                case CppTokens.CPP_EQUALITY     : /* FALL-THROUGH */
                case CppTokens.CPP_INEQUALITY   : return PREC_EQ      ;

                case CppTokens.CPP_LESS         : /* FALL-THROUGH */
                case CppTokens.CPP_LESSEQUAL    : /* FALL-THROUGH */
                case CppTokens.CPP_GREATER      : /* FALL-THROUGH */
                case CppTokens.CPP_GREATEREQUAL : return PREC_CMP     ;

                case CppTokens.CPP_LSHIFT       : /* FALL-THROUGH */
                case CppTokens.CPP_RSHIFT       : return PREC_SHIFT   ;

                case CppTokens.CPP_PLUS         : /* FALL-THROUGH */
                case CppTokens.CPP_MINUS        : return PREC_ADDITIVE;

                case CppTokens.CPP_STAR         : /* FALL-THROUGH */
                case CppTokens.CPP_FSLASH       : /* FALL-THROUGH */
