    ...
    private static class Parser {
        ...
        Value parseExpression(int minPrec) {
            ...
            while (true) {
                ...
                if (CppTokens.CPP_QUESTION.equals(op.type)) {
                    ...
                    if (!check(CppTokens.CPP_COLON)) {
                        throw new SyntaxException("Missing ':' in ternary operator");
                    }
                    consume(); // eat ':'
        ...
        private Value parseUnary() {
            ...
            if (CppTokens.CPP_EXCLAMATION.equals(t.type)) {
                consume();
                Value v = parseExpression(PREC_UNARY - 1);
                try { return v.logicalNot(); } catch (Exception e) { return new Value(0, e); }
            }
        ...
        private Value parsePrimary() {
            ...
            if (CppTokens.CPP_INTEGER.equals(t.type)) {
                consume();
                try { return Value.parse((String) t.value); }
                catch (Exception e) { return new Value(0, e); }
            }
            ...
            if (CppTokens.CPP_LPAREN.equals(t.type)) {
                ...
                if (!check(CppTokens.CPP_RPAREN)) {
                    throw new SyntaxException("Missing ')' in expression");
                }
                consume();
                return v;
            }
            ...
            if (CppTokens.CPP_ID.equals(t.type)) {
                ...
                if (check(CppTokens.CPP_LPAREN)) {
                    ...
                    if (!check(CppTokens.CPP_RPAREN)) {
                        throw new SyntaxException("Missing ')' in function call to " + name);
                    }
                    consume(); // eat ')'
                    Function<Value, Value> fn = functions.get(name);
                    if (fn == null) {
                        return new Value(0, new SyntaxException("Unknown function " + name));
                    }
                ...
                if (val == null && !identifiers.containsKey(name)) {
                    return new Value(0, new SyntaxException("Unknown identifier " + name));
                }
        ...
        private int infixPrec(LexToken t) {
            switch (t.type) {
                case CppTokens.CPP_COMMA:        return PREC_COMMA;
                case CppTokens.CPP_QUESTION:     return PREC_TERNARY;
                case CppTokens.CPP_LOGICALOR:    return PREC_LOR;
                case CppTokens.CPP_LOGICALAND:   return PREC_LAND;
                case CppTokens.CPP_BAR:          return PREC_BITOR;
                case CppTokens.CPP_HAT:          return PREC_XOR;
                case CppTokens.CPP_AMPERSAND:    return PREC_BITAND;
                case CppTokens.CPP_EQUALITY:
                case CppTokens.CPP_INEQUALITY:   return PREC_EQ;
                case CppTokens.CPP_LESS:
                case CppTokens.CPP_LESSEQUAL:
                case CppTokens.CPP_GREATER:
                case CppTokens.CPP_GREATEREQUAL: return PREC_CMP;
                case CppTokens.CPP_LSHIFT:
                case CppTokens.CPP_RSHIFT:       return PREC_SHIFT;
                case CppTokens.CPP_PLUS:
                case CppTokens.CPP_MINUS:        return PREC_ADDITIVE;
                case CppTokens.CPP_STAR:
                case CppTokens.CPP_FSLASH:
                case CppTokens.CPP_PERCENT:      return PREC_MUL;
                default: return -1;
            }
        }
        ...
        private Value applyBinop(LexToken op, Value left, Value right) {
            try {
                switch (op.type) {
                    case CppTokens.CPP_STAR:          return left.mul(right);
                    case CppTokens.CPP_FSLASH:        return left.div(right);
                    case CppTokens.CPP_PERCENT:       return left.mod(right);
                    case CppTokens.CPP_PLUS:          return left.add(right);
                    case CppTokens.CPP_MINUS:         return left.sub(right);
                    case CppTokens.CPP_LSHIFT:        return left.lshift(right);
                    case CppTokens.CPP_RSHIFT:        return left.rshift(right);
                    case CppTokens.CPP_LESS:          return left.lt(right);
                    case CppTokens.CPP_LESSEQUAL:     return left.le(right);
                    case CppTokens.CPP_GREATER:       return left.gt(right);
                    case CppTokens.CPP_GREATEREQUAL:  return left.ge(right);
                    case CppTokens.CPP_EQUALITY:      return left.eq(right);
                    case CppTokens.CPP_INEQUALITY:    return left.ne(right);
                    case CppTokens.CPP_AMPERSAND:     return left.and(right);
                    case CppTokens.CPP_HAT:           return left.xor(right);
                    case CppTokens.CPP_BAR:           return left.or(right);
                    case CppTokens.CPP_LOGICALAND:    return left.logicalAnd(right);
                    case CppTokens.CPP_LOGICALOR:     return left.logicalOr(right);
                    case CppTokens.CPP_COMMA:         return left.comma(right);
                    default:
