...
public final class Value {
    ...
    private static final BigInteger BIG_MASK = BigInteger.ONE.shiftLeft(
        64
    ).subtract(
        BigInteger.ONE
    );
    ...
    public static Value parse(String input)
    {
        ...
        if( s.startsWith("0x") || s.startsWith("0X") ) {
            ...
            while( !s.isEmpty() ) {
                ...
                if(last == 'u' || last == 'U') { unsigned = true; s = s.substring(
                    0, s.length() - 1
                ); }
    ...
    private static String stripSuffixes(String s)
    {
        while( !s.isEmpty() ) {
            char last = s.charAt( s.length() - 1 );
            if(last == 'u' || last == 'U' || last == 'l' || last == 'L') s = s.substring(
                0, s.length() - 1
            );
            else break;
        } // while
