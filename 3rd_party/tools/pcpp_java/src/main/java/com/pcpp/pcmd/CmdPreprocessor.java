...
public class CmdPreprocessor extends Preprocessor {
    ...
    private static String[] expandBraces(String[] argv)
    {
        ...
        for(String arg : argv) {
            if( arg.startsWith("{") && arg.endsWith("}") && arg.indexOf(',') >= 0 ) {
                ...
                for( String part : inner.split(",", -1) ) expanded.add(part);
            }
            else {
                expanded.add(arg);
            }
    ...
    private void setupAndProcess()
    {
        ...
        if( line_directive != null && ( line_directive.equalsIgnoreCase("nothing") ||
                                       line_directive.equalsIgnoreCase(
                                           "none"
                                       ) || line_directive.isEmpty() ) ) line_directive = null;

        if(args.passthru_includes != null) passthru_includes = Pattern.compile(
            args.passthru_includes
        );
        ...
        if(args.assume_input_encoding != null) assume_encoding = args.assume_input_encoding;
        ...
        // Process -U undefines
        for(String d : args.undefines) undef(d);

        // Process -I includes
        for(String d : args.includes) add_path(d);
        ...
        try {
            ...
            if( args.inputPaths.isEmpty() || args.inputPaths.equals(
                ...
                try ( Reader rdr = args.assume_input_encoding != null ?
                                  new InputStreamReader(
                                      new FileInputStream(inputPath), args.assume_input_encoding
                                  ) :
                ...
                StringBuilder sb = new StringBuilder();
                for(String p : args.inputPaths) sb.append("#include \"").append(p).append("\"\n");
            ...
            if( !( output instanceof OutputStreamWriter && ( (OutputStreamWriter) output ).getEncoding() != null &&
                  System.out.equals(output) ) ) output.close();

        }
        catch(Exception e) {
            e.printStackTrace(System.err);
            if(lastdirective != null) System.err.printf(
                "%nINTERNAL PREPROCESSOR ERROR AT AROUND %s:%d, FATALLY EXITING NOW%n",
        ...
        if(args.time_report) {
            ...
            for( int n = 0; n < include_times.size(); ++n ) {
                FileInclusionTime it = include_times.get(n);
                     if(n == 0)        System.out.printf("top level: %f seconds%n", it.elapsed);
                else if(it.depth == 1) System.out.printf(
                    "\n %s: %f seconds (%.0f%%)%n",
                    it.included_path,
                    it.elapsed,
                    ...
                    "%s%s: %f seconds%n", spaces(it.depth), it.included_path, it.elapsed
                );
            } // for
        } // if
        ...
        if(args.filetimes != null) {
            try ( PrintWriter fw = new PrintWriter( new FileWriter(args.filetimes) ) ) {
                ...
                for(FileInclusionTime it : include_times) {
                    while( it.depth < currentfiles.size() ) currentfiles.remove(
                        currentfiles.size() - 1
                    );
                    if( it.depth > currentfiles.size() - 1 ) currentfiles.add(it.included_abspath);
                    String path2 = currentfiles.isEmpty() ? null : currentfiles.get(
                        currentfiles.size() - 1
                    );
                    if(path2 != null) {
    ...
    {
        if(args.passthru_unfound_includes) throw new OutputDirective(Action.IgnoreAndPassThrough);
    ...
    {
        if( !args.undefines.isEmpty() && args.undefines.contains(tok.value) ) return Boolean.FALSE;
        if(args.passthru_undefined_exprs) return null; // Pass through as expanded as possible
    ...
    {
        if( !args.undefines.isEmpty() && args.undefines.contains(
            ident
        ) ) return super.on_unknown_macro_in_expr(
            ident
        );
        if(args.passthru_undefined_exprs) return null; // Pass through as expanded as possible
    ...
    @Override
    public java.util.function.Function<com.pcpp.evaluator.Value, com.pcpp.evaluator.Value> on_unknown_macro_function_in_expr(
        String ident
    )
    {
        if( !args.undefines.isEmpty() && args.undefines.contains(
            ident
        ) ) return super.on_unknown_macro_function_in_expr(
            ident
        );
        if(args.passthru_undefined_exprs) return null;
    ...
    )
    {
        ...
        if(ifpassthru) {
            ...
            if( !bypass_ifpassthru && ( "define".equals(dval) || "undef".equals(dval) ) )
                if( !toks.isEmpty() && !toks.get(
                    0
                ).value.equals(
                    potential_include_guard
                ) ) throw new OutputDirective(
                    Action.IgnoreAndPassThrough
                );
        } // if
        if( ( "define".equals(dval) || "undef".equals(dval) ) && !args.nevers.isEmpty() )
            if( !toks.isEmpty() && args.nevers.contains(
                toks.get(0).value
            ) ) throw new OutputDirective(
                Action.IgnoreAndPassThrough
            );
    ...
    {
        if(ifpassthru) return null; // Pass through
    ...
    {
        if(args.passthru_comments) return Boolean.TRUE; // Pass through
    ...
    private static Args parseArgs(String[] argv)
    {
        ...
        while(i < argv.length) {
            ...
            switch(arg) {
                ...
                case "-h":
                    printHelp(); System.exit(0); break;

                case "--version":
                    System.out.println("pcpp " + VERSION); System.exit(0); break;
                ...
                case "--passthru-defines":
                    a.passthru_defines          = true; break;

                case "--passthru-unfound-includes":
                    a.passthru_unfound_includes = true; break;

                case "--passthru-unknown-exprs":
                    a.passthru_undefined_exprs  = true; break;

                case "--passthru-comments":
                    a.passthru_comments         = true; break;

                case "--passthru-magic-macros":
                    a.passthru_magic_macros     = true; break;

                case "--passthru-includes":
                    if(++i < argv.length) a.passthru_includes = argv[i];
                    break;

                case "--disable-auto-pragma-once":
                    a.auto_pragma_once_disabled = true; break;
