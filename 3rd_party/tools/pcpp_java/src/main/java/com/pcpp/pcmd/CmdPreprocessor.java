...
public class CmdPreprocessor extends Preprocessor {
    ...
    private void setupAndProcess()
    {
        ...
        if( line_directive != null && (line_directive.equalsIgnoreCase( "nothing" ) ||
                                       line_directive.equalsIgnoreCase( "none" ) || line_directive.isEmpty() ) ) line_directive = null;

        if( args.passthru_includes != null ) passthru_includes = Pattern.compile( args.passthru_includes );
        ...
        try {
            ...
            if( lastdirective != null ) System.err.printf( "%nINTERNAL PREPROCESSOR ERROR AT AROUND %s:%d, FATALLY EXITING NOW%n",
                ...
                for( FileInclusionTime it : include_times ) {
                    ...
                    if( path2 != null ) {
                        ...
                        if( it.elapsed > 0 && currentfiles.size() > 1 ) {
                            ...
                            if( filetimesMap.containsKey( parent ) ) filetimesMap.get( parent )[1] -= it.elapsed;
                        }
                    }
                }
    ...
    {
        if( !args.undefines.isEmpty() && args.undefines.contains( ident ) ) return super.on_unknown_macro_in_expr( ident );
        if( args.passthru_undefined_exprs ) {
            return null; // Pass through as expanded as possible
        }
    ...
    {
        if( !args.undefines.isEmpty() && args.undefines.contains( ident ) ) return super.on_unknown_macro_function_in_expr( ident );
        ...
        boolean ifpassthru, List<LexToken> precedingtoks )
    {
        ...
        if( ifpassthru ) {
            if( "if".equals( dval ) || "elif".equals( dval ) || "else".equals( dval ) || "endif".equals( dval ) ) bypass_ifpassthru = toks.stream().anyMatch( t ->
            ...
            if( !bypass_ifpassthru && ("define".equals( dval ) || "undef".equals( dval ) ) )
                if( !toks.isEmpty() && !toks.get( 0 ).value.equals( potential_include_guard ) ) throw new OutputDirective( Action.IgnoreAndPassThrough );
        }
        if( ("define".equals( dval ) || "undef".equals( dval ) ) && !args.nevers.isEmpty() )
            if( !toks.isEmpty() && args.nevers.contains( toks.get( 0 ).value ) ) throw new OutputDirective( Action.IgnoreAndPassThrough );
            ...
            switch( arg ) {
                ...
                case "-o":
                    ...
                    if( i + 1 < argv.length ) {
                        ...
                        while( i + 1 < argv.length && !argv[i + 1].startsWith( "-" ) ) a.outputPaths.add( argv[++i] );
                    }
                    break;
