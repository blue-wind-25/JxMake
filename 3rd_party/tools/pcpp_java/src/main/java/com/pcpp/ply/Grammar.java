...
public class Grammar {
    ...
    {
        if( Productions.size() > 1 ) throw new GrammarError(
            "Must call set_precedence() before add_production()"
        );
        if( Precedence.containsKey(
            term
        ) ) throw new GrammarError(
            "Precedence already specified for terminal '" + term + "'"
        );
        if( !assoc.equals(
            "left"
        ) && !assoc.equals(
    ...
    )
    {
        ...
        if( expandedSyms.contains("%prec") ) {
            ...
            if( precname != null && Precedence.containsKey(
                precname
            ) ) prodprec = Precedence.get(
                precname
            );
            else prodprec = new String[] { "right", "0" };
        }
    ...
    public void set_start(String start)
    {
        ...
        if( !Nonterminals.containsKey(
            start
        ) ) throw new GrammarError(
            "start symbol '" + start + "' undefined"
        );
    ...
    public List<String> infinite_cycles()
    {
        ...
        for( Map.Entry<String, Boolean> e : terminates.entrySet() )
            if( !e.getValue() ) {
                String s = e.getKey();
                if( Prodnames.containsKey(
                    s
                ) || Terminals.containsKey(
                    s
                ) || s.equals(
                    "error"
                ) ) infinite.add(
    ...
    public List<String[]> unused_precedence()
    {
        ...
        for( Map.Entry<String, String[]> e : Precedence.entrySet() ) {
            ...
            if( !Terminals.containsKey(
                termname
            ) && !UsedPrecedence.contains(
                termname
            ) ) unused.add(
                new String[] {termname, e.getValue()[0]}
            );
        } // for
    ...
    public List<String> _first(List<String> beta)
    {
        ...
        for(String x : beta) {
            ...
            if(fx != null) {
                for(String f : fx) {
                         if( f.equals("<empty>") ) xProducesEmpty = true;
                    else if( !result.contains(f) ) result.add(f);
                }
            }
            if(!xProducesEmpty) {
