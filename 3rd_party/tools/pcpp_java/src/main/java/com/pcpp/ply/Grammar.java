...
public class Grammar {
    ...
    public Grammar( List<String> terminals )
    {
        ...
        for( String t : terminals ) Terminals.put( t, new ArrayList<>() );
    ...
    {
        if( Productions.size() > 1 ) throw new GrammarError( "Must call set_precedence() before add_production()" );
        if( Precedence.containsKey( term ) ) throw new GrammarError( "Precedence already specified for terminal '" + term + "'" );
        if( !assoc.equals( "left" ) && !assoc.equals( "right" ) && !assoc.equals( "nonassoc" ) ) throw new GrammarError( "Associativity must be one of 'left', 'right', or 'nonassoc'" );
    ...
    {
        if( Terminals.containsKey( prodname ) ) throw new GrammarError( file + ":" + line +
            ": Illegal rule name '" + prodname + "'. Already defined as a token" );
        if( prodname.equals( "error" ) ) throw new GrammarError( file + ":" + line +
            ": Illegal rule name 'error'. error is a reserved word" );
        if( !IS_IDENTIFIER.matcher( prodname ).matches() ) throw new GrammarError( file + ":" + line +
        ...
        for( int n = 0; n < expandedSyms.size(); n++ ) {
            ...
            if( s.length() > 0 && (s.charAt( 0 ) == '\'' || s.charAt( 0 ) == '"') ) {
                ...
                if( !Terminals.containsKey( c ) ) Terminals.put( c, new ArrayList<>() );
        ...
        if( expandedSyms.contains( "%prec" ) ) {
            ...
            if( precIdx == expandedSyms.size() - 1 ) throw new GrammarError( file + ":" + line +
                ": Syntax error. Nothing follows %prec" );
            if( precIdx != expandedSyms.size() - 2 ) throw new GrammarError( file + ":" + line +
            ...
            if( prodprec == null ) throw new GrammarError( file + ":" + line +
        ...
        if( !Nonterminals.containsKey( prodname ) ) Nonterminals.put( prodname, new ArrayList<>() );
        ...
        for( String t : expandedSyms ) {
            if( Terminals.containsKey( t ) ) {
                ...
                if( !Nonterminals.containsKey( t ) ) Nonterminals.put( t, new ArrayList<>() );
        ...
        if( !Prodnames.containsKey( prodname ) ) Prodnames.put( prodname, new ArrayList<>() );
    ...
    public void set_start( String start )
    {
        ...
        if( !Nonterminals.containsKey( start ) ) throw new GrammarError( "start symbol '" + start + "' undefined" );
    ...
    private void markReachableFrom( String s, Set<String> reachable )
    {
        ...
        if( prods != null )
            for( Production p : prods )
                for( String r : p.prod ) markReachableFrom( r, reachable );
    }
    ...
    public List<String> infinite_cycles()
    {
        ...
        for( Map.Entry<String, Boolean> e : terminates.entrySet() )
            if( !e.getValue() ) {
                String s = e.getKey();
                if( Prodnames.containsKey( s ) || Terminals.containsKey( s ) || s.equals( "error" ) ) infinite.add( s );
            }
        return infinite;
    }
    ...
    public List<Object[]> undefined_symbols()
    {
        ...
        for( Production p : Productions ) {
            if( p == null ) continue;
            for( String s : p.prod )
                if( !Prodnames.containsKey( s ) && !Terminals.containsKey( s ) && !s.equals( "error" ) ) result.add( new Object[] {s, p} );
        }
        return result;
    }
    ...
    public List<String> unused_terminals()
    {
        ...
        for( Map.Entry<String, List<Integer> > e : Terminals.entrySet() )
            if( !e.getKey().equals( "error" ) && e.getValue().isEmpty() ) unused.add( e.getKey() );
        return unused;
    }
    ...
    public List<Production> unused_rules()
    {
        ...
        for( Map.Entry<String, List<Integer> > e : Nonterminals.entrySet() )
            if( e.getValue().isEmpty() ) unused.add( Prodnames.get( e.getKey() ).get( 0 ) );
        return unused;
    }
    ...
    public List<String[]> unused_precedence()
    {
        ...
        for( Map.Entry<String, String[]> e : Precedence.entrySet() ) {
            ...
            if( !Terminals.containsKey( termname ) && !UsedPrecedence.contains( termname ) ) unused.add( new String[] {termname, e.getValue()[0]} );
        }
        return unused;
    }
    ...
    public List<String> _first( List<String> beta )
    {
        ...
        if( allProduceEmpty && !beta.isEmpty() ) result.add( "<empty>" );
        return result;
    }
    ...
    public Map<String, List<String> > compute_first()
    {
        ...
        for( String t : Terminals.keySet() ) First.put( t, Collections.singletonList( t ) );
        First.put( "$end", Collections.singletonList( "$end" ) );

        for( String n : Nonterminals.keySet() ) First.put( n, new ArrayList<>() );
    ...
    public Map<String, List<String> > compute_follow( String start )
    {
        ...
        for( String k : Nonterminals.keySet() ) Follow.put( k, new ArrayList<>() );
        ...
        for( Production p : Productions ) {
            ...
            for( int i = 0; i <= p.len; i++ ) {
                ...
                if( i > p.len ) {
                    ...
                    if( i > 0 ) lri.lr_before = p.prod.get( i - 1 );
                }
                if( lastlri != null ) lastlri.lr_next = lri;
                if( lri == null ) break;
                lr_items.add( lri );
                lastlri = lri;
            }
