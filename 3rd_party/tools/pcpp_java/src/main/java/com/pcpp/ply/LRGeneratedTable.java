...
public class LRGeneratedTable extends LRTable {
    ...
    public                                 LRGeneratedTable( Grammar grammar, String method, PlyLogger log ) {
        if( !method.equals( "SLR" ) && !method.equals( "LALR" ) ) throw new LALRError( "Unsupported method: " + method );
                ...
                if( p.lr_index == p.len - 1 ) {
                    ...
                    if( grammar.Terminals.containsKey( a ) ) {
                        ...
                        if( j != null && j >= 0 ) {
                            ...
                            if( r != null ) {
                                if( r > 0 ) {
                                    if( !r.equals( j ) ) throw new LALRError( "Shift/shift conflict in state " + st );
                                }
                                else if( r < 0 ) {
    ...
    public void add_lalr_lookaheads( List<List<LRItem> > C )
    {
        ...
        for( int i = 0; i < numTrans; i++ ) drSets.add( dr_relation_full( C, transStates.get( i )[0], transSyms.get( i ), nullable ) );
                ...
                while( lr_index < p.len - 1 ) {
                    ...
                    if( dtrans.containsKey( tkey ) ) {
                        ...
                        if( suffixNullable ) {
                            ...
                            if( !includesRel.get( includedIdx ).contains( ti ) ) includesRel.get( includedIdx ).add( ti );
                        }
                    }
