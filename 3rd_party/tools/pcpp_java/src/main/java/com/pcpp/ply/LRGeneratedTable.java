...
public class LRGeneratedTable extends LRTable {
    ...
    public LRGeneratedTable(Grammar grammar, String method, PlyLogger log) {
        if (!method.equals("SLR") && !method.equals("LALR")) {
            throw new LALRError("Unsupported method: " + method);
        }
        ...
        for (Production p : grammar.Productions) {
            if (p == null) {
                this.lr_productions.add(null);
            } else {
                this.lr_productions.add(new MiniProduction(
    ...
    public List<LRItem> lr0_goto(List<LRItem> I, String x) {
        ...
        for (LRItem p : I) {
            LRItem n = p.lr_next;
            if (n != null && x.equals(n.lr_before)) {
                gs.add(n);
            }
        }
    ...
    public List<List<LRItem>> lr0_items() {
        ...
        while (i < C.size()) {
            ...
            for (String x : asyms) {
                ...
                if (g.isEmpty() || lr0_cidhash.containsKey(System.identityHashCode(g))) {
                    continue;
                }
    ...
    public List<String> dr_relation(List<List<LRItem>> C, int[] trans, String N, Set<String> nullable) {
        List<String> terms = new ArrayList<>();
        List<LRItem> g = lr0_goto(C.get(trans[0]), N);
        for (LRItem p : g) {
            if (p.lr_index < p.len - 1) {
                String a = p.prod.get(p.lr_index + 1);
                if (grammar.Terminals.containsKey(a) && !terms.contains(a)) {
                    terms.add(a);
                }
            }
        }
        if (trans[0] == 0 && N.equals(grammar.Productions.get(0).prod.get(0))) {
            terms.add("$end");
        }
        return terms;
    }
                        ...
                        for (String a : laheads) {
                            ...
                            if (r != null) {
                                ...
                                if (r > 0) {
                                    ...
                                    String[] sprec_pair = Precedence.getOrDefault(a, new String[]{"right", "0"});
                                    ...
                                    if (slevel < rlevel || (slevel == rlevel && rprec.equals("left"))) {
                                        // Reduce wins
                    ...
                    if (grammar.Terminals.containsKey(a)) {
                        ...
                        if (j != null && j >= 0) {
                            ...
                            if (r != null) {
                                if (r > 0) {
                                    if (!r.equals(j)) {
                                        throw new LALRError("Shift/shift conflict in state " + st);
                                    }
                                } else if (r < 0) {
                                    ...
                                    String[]   rprec_pair = rProd != null ? rProd.prec : new String[]{"right","0"};
                                    String     rprec  = rprec_pair[0];
                                    int        rlevel = Integer.parseInt(rprec_pair[1]);

                                    if (slevel > rlevel || (slevel == rlevel && rprec.equals("right"))) {
                                        // Shift wins
    ...
    public void add_lalr_lookaheads(List<List<LRItem>> C) {
        ...
        for (int i = 0; i < numTrans; i++) {
            drSets.add(dr_relation_full(C, transStates.get(i)[0], transSyms.get(i), nullable));
        }
        ...
        for (int i = 0; i < numTrans; i++) {
            long key = (long) transStates.get(i)[0] << 32 | (transSyms.get(i).hashCode() & 0xFFFFFFFFL);
            dtrans.put(key, i);
        }
                ...
                while (lr_index < p.len - 1) {
                    ...
                    if (dtrans.containsKey(tkey)) {
                        ...
                        if (suffixNullable) {
                            ...
                            if (!includesRel.get(includedIdx).contains(ti)) {
                                includesRel.get(includedIdx).add(ti);
                            }
                        }
                    }
            ...
            for (int[] lb : lookback.get(ti)) {
                ...
                for (LRItem r : C.get(lbState)) {
                    if (r.number == lbProdNum && r.lr_index == r.len - 1) {
                        List<String> laheads = r.lookaheads.computeIfAbsent(lbState, k -> new ArrayList<>());
                        for (String a : fs) {
    ...
    private List<String> dr_relation_full(List<List<LRItem>> C, int state, String N, Set<String> nullable) {
        List<String> terms = new ArrayList<>();
        List<LRItem> g = lr0_goto(C.get(state), N);
        for (LRItem p : g) {
            if (p.lr_index < p.len - 1) {
     ...
     */
    private List<List<String>> digraph(int n,
                                       List<List<Integer>> R,
                                       List<List<String>> FP) {
        int[] N = new int[n];
        ...
        // Make mutable copies
        for (int i = 0; i < n; i++) {
            F.set(i, new ArrayList<>(F.get(i)));
        }
        Deque<Integer> stack = new ArrayDeque<>();
        for (int x = 0; x < n; x++) {
            if (N[x] == 0) {
                traverse(x, N, stack, F, R);
            }
        }
        return F;
    }

    private void traverse(int x, int[] N, Deque<Integer> stack,
                          List<List<String>> F, List<List<Integer>> R) {
        stack.push(x);
        int d = stack.size();
        N[x] = d;
