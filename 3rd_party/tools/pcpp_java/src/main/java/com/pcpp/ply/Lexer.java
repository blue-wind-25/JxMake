...
public class Lexer {
    ...
    public void begin(String state) {
        if (!lexstatere.containsKey(state)) {
            throw new IllegalArgumentException("Undefined lexer state: " + state);
        }
    ...
    public LexToken token() {
        ...
        this.lexpos = pos + 1;
        if (lexdata == null) {
            throw new RuntimeException("No input string given with input()");
        }
        return null;
    }
    ...
    public Iterable<LexToken> tokens() {
        return () -> new java.util.Iterator<LexToken>() {
            ...
            private LexToken advance() {
                try { return token(); }
                catch (LexError e) { throw new RuntimeException(e); }
            }
    ...
    public static class Builder {
        ...
        public Lexer build() {
            ...
            if (errorCallback != null) {
                spec.setErrorCallback(errorCallback);
            }
