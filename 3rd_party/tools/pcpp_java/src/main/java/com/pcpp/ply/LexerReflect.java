...
public class LexerReflect {
    ...
    private void getTokens(LexerSpec target)
    {
        ...
        for(String n : arr) {
            ...
            if( !seen.add(n) ) log.warning("Token '%s' multiply defined", n);
        } // for
    ...
    private void getLiterals(LexerSpec target)
    {
        ...
        if(val instanceof String) {
            String s = (String)val;
            for( char c : s.toCharArray() ) target.addLiteral(c);
        }
        else if( val instanceof char[] ) {
            for( char c : ( char[] ) val ) target.addLiteral(c);
        }
    ...
    private void getStates(LexerSpec target, Map<String, String> stateInfo)
    {
        ...
        for( String[] pair : arr ) {
            if(pair.length != 2) {
                log.error(
                    "Invalid state specifier. Each entry must be {\"name\",\"inclusive|exclusive\"}"
                );
                hasError = true;
                continue;
            } // if
    ...
    private void getRules(LexerSpec target, Map<String, String> stateInfo)
    {
        ...
        for( Field f : cls.getFields() )
            if( f.getName().startsWith("t_") ) tFields.add(f);
        ...
        for( Method m : cls.getMethods() )
            if( m.getName().startsWith("t_") ) tMethods.add(m);
        ...
        for(Field f : tFields) {
            ...
            if( "ignore".equals(tokenName) ) {
                for(String s : states) target.setIgnore(s, pattern);
                continue;
            }
        ...
        for(Method m : tMethods) {
            ...
            if( "error".equals(tokenName) ) {
                ...
                for(String s : states) target.setErrorCallback(s, cb);
                continue;
            } // if
            ...
            if( "eof".equals(tokenName) ) {
                ...
                for(String s : states) target.setEofCallback(s, cb);
                continue;
            } // if
            ...
            TokenCallback cb             = tok -> {
                try {
                    ...
                    throw new RuntimeException(
                        "Error in rule handler " + ruleMethod.getName(), ex
                    );
                }
            };
    ...
    private String[] stateToken(String s, Set<String> names)
    {
        ...
        for(; i < parts.length; ++i)
            if( !names.contains( parts[i] ) && !"ANY".equals( parts[i] ) ) break;
    ...
    private String[] stateTokenStates(String s, Set<String> allNames)
    {
        ...
        for(; i < parts.length; ++i) {
                 if( allNames.contains( parts[i] ) ) stateList.add( parts[i] );
            else if( "ANY".equals( parts[i] ) )      stateList.addAll(allNames);
            else                                     break;
        }
        if( stateList.isEmpty() ) stateList.add("INITIAL");
     ...
     */
    private String stateSpecificRuleName(
        String      originalName,
        String      state,
        String[]    allStates,
        Set<String> allStateNames
    )
    {
        if( allStates.length == 1 && "INITIAL".equals(state) ) return originalName;
        ...
        int      i     = 1;
        while( i < parts.length && ( allStateNames.contains(
            parts[i]
        ) || "ANY".equals(
            parts[i]
        ) ) ) i++;
    ...
    {
        try {
            Pattern p = Pattern.compile("(?<" + name + ">" + pattern + ")", Pattern.COMMENTS);
    ...
    private static String[] toStringArray(Object val)
    {
        ...
        if(val instanceof List) {
            ...
            for(int i = 0; i < arr.length; ++i) arr[i] = list.get(i).toString();
            return arr;
        }
