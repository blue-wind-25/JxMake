...
public class YaccBuilder {
    ...
    )
    {
        ...
        // 1. Validate tokens
        if( spec.tokens == null || spec.tokens.isEmpty() ) throw new YaccError(
        ...
        for(String t : spec.tokens)
            if( t.equals(
                ...
                "Illegal token name 'error'. It is a reserved word"
            );
        ...
        for( String term : grammar.unused_terminals() ) errorlog.warning(
            "Token '%s' defined, but not used", term
        );

        for( Production prod : grammar.unused_rules() ) errorlog.warning(
        ...
        for( String[] prec : grammar.unused_precedence() ) {
            errorlog.error(
                "Precedence rule '%s' defined for unknown symbol '%s'",
                prec[1], prec[0]
            );
            errors = true;
        } // for
        ...
        if(CHECK_RECURSION) {
            for( String u : grammar.find_unreachable() ) errorlog.warning(
                "Symbol '%s' is unreachable", u
            );
        ...
        for( String ps : doc.split("\n", -1) ) {
            ...
            if( parts[0].equals("|") ) {
                ...
                if( !assign.equals(
                    ...
                    "::="
                ) ) throw new YaccError(file + ":" + dline +
