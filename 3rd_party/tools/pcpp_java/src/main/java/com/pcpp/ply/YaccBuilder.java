...
public class YaccBuilder {
    ...
    public static LRParser build(GrammarSpec spec, String method,
        ...
        // 1. Validate tokens
        if (spec.tokens == null || spec.tokens.isEmpty()) {
            throw new YaccError("No token list is defined");
        }
        for (String t : spec.tokens) {
            if (t.equals("error")) {
                throw new YaccError("Illegal token name 'error'. It is a reserved word");
            }
        }
        ...
        for (GrammarRule rule : spec.rules) {
            if (rule.action != null) {
                actionMap.put(rule.name, rule.action);
            }
        }
        ...
        for (String term : grammar.unused_terminals()) {
            errorlog.warning("Token '%s' defined, but not used", term);
        }

        for (Production prod : grammar.unused_rules()) {
        ...
        if (CHECK_RECURSION) {
            for (String u : grammar.find_unreachable()) {
    ...
    public static List<Object[]> parseGrammarString(String doc, String file, int line) {
        ...
        for (String ps : doc.split("\n", -1)) {
            ...
            if (parts[0].equals("|")) {
                ...
                if (lastp == null) {
                    throw new YaccError(file + ":" + dline + ": Misplaced '|'");
                }
                prodname = lastp;
                ...
                if (!assign.equals(":") && !assign.equals("::=")) {
                    throw new YaccError(file + ":" + dline +
