...
public class Preprocessor extends PreprocessorHooks {
    ...
    protected Object[] collect_args(List<LexToken> tokenlist, boolean ignore_errors) {
        ...
        if (i < tokenlen && "(".equals(tokenlist.get(i).value)) {
            ...
            return new Object[]{0, Collections.emptyList(), Collections.emptyList()};
        }
        i++;
        ...
        while (i < tokenlen) {
            ...
            if ("(".equals(t.value)) {
                ...
                if (nesting == 0) {
                    ...
                    positions.add(i);
                    return new Object[]{i + 1, args, positions};
                }
                current_arg.add(t);
    ...
    protected void macro_prescan(Macro macro) {
        ...
        if (macro.arglist != null) {
            for (int idx = 0; idx < macro.arglist.size(); idx++) {
                arglist_index.put(macro.arglist.get(idx), idx);
            }
        }
        ...
        while (i < macro.value.size()) {
            ...
            if (t_ID.equals(tok.type) && arglist_index.containsKey(tok.value)) {
                ...
                if (macro.variadic && i > 0 && ",".equals(macro.value.get(i - 1).value) &&
                        ...
                        macro.vararg != null && macro.vararg.equals(macro.value.get(i + 1).value)) {
                    macro.var_comma_patch.add(i - 1);
                }
            }
            i++;
        }
    ...
    protected List<LexToken> macro_expand_args(Macro macro, List<List<LexToken>> args) {
        ...
        for (int[] strp : macro.str_patch) {
            ...
            if (!str_expansion.containsKey(argnum)) {
                ...
                for (LexToken t : tokens2) {
                    if (t_WS.contains(t.type) && !t_LINECONT.equals(t.type)) {
                        t.value = " ";
                    }
                }
        ...
        for (Object[] patch : macro.patch) {
            ...
            if ("t".equals(ptype)) {
                ...
                if (!expanded.containsKey(argnum)) {
                    expanded.put(argnum, expand_macros(new ArrayList<>(args.get(argnum)), new ArrayList<>()));
                }
                rep.subList(pIdx, pIdx + 1).clear();
            ...
            while (ki < rep.size()) {
                ...
                if (t != null && t.type == null) {
                    ...
                    if (!toks2.isEmpty()) {
                        ...
                        for (int m = 1; m < toks2.size(); m++) {
                            rep.add(ki + m, toks2.get(m));
                        }
                        ki += toks2.size();
                    } else {
                        rep.remove(ki);
    ...
    protected Object[] evalexpr(List<LexToken> tokens) {
        ...
        Function<List<LexToken>, List<LexToken>> replace_defined = (toks) -> {
            ...
            while (i < toks.size()) {
                if (t_ID.equals(toks.get(i).type) && "defined".equals(toks.get(i).value)) {
                    ...
                    // Remove tokens i+1..j
                    if (j + 1 <= toks.size()) {
                        toks.subList(i + 1, Math.min(j + 1, toks.size())).clear();
                    }
                }
                i++;
            }
        ...
        if (tokens.isEmpty()) return new Object[]{0L, null};
        ...
        return new Object[]{result, partial_expansion[0] ? tokens : null};
    }
    ...
    public Iterable<LexToken> parsegen(String input, String source, String abssource) {
            ...
            for (Object[] rewrite : rewrite_paths) {
                ...
                if (m.find()) {
                    ...
                    if (!File.separator.equals("/")) {
                        rewritten_source = rewritten_source.replace(File.separator, "/");
                    }
                    break;
                }
            }
        ...
        if (expand_filemacro) {
            define("__FILE__ \"" + rewritten_source + "\"");
        }
        ...
        for (List<LexToken> x : lines) {
                    ...
                    switch (name) {
                            ...
                            if (enable) {
                                ...
                                if (include_guard[0] != null && "0".equals(include_guard[1])) {
                                    if (!args.isEmpty() && include_guard[0].equals(args.get(0).value) && args.size() == 1) {
                                        include_guard[1] = "1";
                                        if (ifpassthru && !ifstack.isEmpty() && !ifstack.peek().ifpassthru) {
                                            ifpassthru = false;
                                        }
                                    }
                                }
                        ...
                        case "include":
                            if (enable) {
                                ...
                                if (!args.isEmpty() && !"<".equals(args.get(0).value) && !t_STRING.equals(args.get(0).type)) {
                                    args = tokenstrip(expand_macros(args, new ArrayList<>()));
                                }
                        ...
                        case "elif":
                            ...
                            if (!ifstack.isEmpty()) {
                                ...
                                if (!dirtokens.isEmpty())
                                    on_error((String) dirtokens.get(0).source, dirtokens.get(0).lineno, "Misplaced #elif");
                            }
                            break;
                        ...
                        case "else":
                            ...
                            if (!ifstack.isEmpty()) {
                                ...
                                if (!dirtokens.isEmpty())
                                    on_error((String) dirtokens.get(0).source, dirtokens.get(0).lineno, "Misplaced #else");
                            }
                            break;
                        ...
                        case "endif":
                            ...
                            if (!ifstack.isEmpty()) {
                                ...
                                if (oldEntry.rewritten) {
                                    throw new OutputDirective(Action.IgnoreAndPassThrough);
                                }
                            } else {
                                if (!dirtokens.isEmpty())
                                    on_error((String) dirtokens.get(0).source, dirtokens.get(0).lineno, "Misplaced #endif");
                            }
                            break;
            ...
            if (!skip_auto_pragma_once_possible_check && auto_pragma_once_possible &&
                    ifstack.isEmpty() && !all_whitespace) {
        ...
        if (auto_pragma_once_possible && include_guard[0] != null && "1".equals(include_guard[1])) {
            include_once.put(this.source, include_guard[0]);
        }
    ...
    public Iterable<LexToken> include(List<LexToken> tokens, List<LexToken> original_line) {
        ...
        if (!"<".equals(tokens.get(0).value) && !t_STRING.equals(tokens.get(0).type)) {
            tokens = tokenstrip(expand_macros(tokens, new ArrayList<>()));
        }
            ...
            for (String p : searchPath) {
                ...
                if (include_once.containsKey(fulliname)) {
                    if (passthru_includes != null) {
                        StringBuilder sb = new StringBuilder();
                        for (LexToken t : tokens) sb.append(t.value);
                        if (passthru_includes.matcher(sb.toString()).find()) {
    ...
    public void undef(List<LexToken> tokens) {
        if (!tokens.isEmpty()) {
            macros.remove(tokens.get(0).value.toString());
        }
    }
        ...
        try {
            ...
            while (!done) {
                ...
                for (int n = toks.size() - 1; n >= 0; n--) {
                    ...
                    if (first_ws == null) {
                        if (t_SPACE.equals(tok3.type) || (tok3.value != null && tok3.value.toString().isEmpty())) {
                            first_ws = n;
                        }
                    } else {
                        if (!t_SPACE.equals(tok3.type) && (tok3.value == null || !tok3.value.toString().isEmpty())) {
                ...
                if (emitlinedirective && line_directive != null) {
                    oh.write(line_directive + " " + lastlineno +
