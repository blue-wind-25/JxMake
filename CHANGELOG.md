...
+ Enhanced and bug‑fixed the Python‑based documentation server in `util/MDXplorer`.

+ Enhanced and bug‑fixed the Java compiler server and its shell scripts (`.sh`, `.ps1`, `.cmd`) in `util/JCS/*`.

+ Added coding‑style rules for human developers and AI CLI/bot formatters in `util/CodingStyle.md/*.md`, along with a Java‑based coding‑style formatter in `util/CodingStyle.md/formatter`.
