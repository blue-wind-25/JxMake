...
{
	if(value) BB_USRT_PORT |=  _BV(BB_USRT_TXD_BIT);
	else      BB_USRT_PORT &= ~_BV(BB_USRT_TXD_BIT);
}


static inline ATTR_ALWAYS_INLINE bool _rxd(void)
{ return BB_USRT_PIN & _BV(BB_USRT_RXD_BIT); }


static inline ATTR_ALWAYS_INLINE void _xck(bool value)
{
	if(value) BB_USRT_PORT |=  _BV(BB_USRT_XCK_BIT);
	else      BB_USRT_PORT &= ~_BV(BB_USRT_XCK_BIT);
}
...
bool bbusrt_begin(uint8_t parityMode, uint8_t numStopBits, bool ssMode, uint32_t baudrate)
{
	...
	BB_USRT_DDR &= ~_BV(BB_USRT_RXD_BIT);
	BB_USRT_DDR |=  _BV(BB_USRT_TXD_BIT) | _BV(BB_USRT_XCK_BIT);
	...
	/*
	__DPRINTF_DECL_PREFIX("bbusrt_begin");

	bool val;
	for(int i = 0; i < 5; ++i) {
		val = false; _txd(val); Delay_MS(100); __DPRINTFS_X( "@@@ %d -> %d"  , val, _rxd() );
		val = true ; _txd(val); Delay_MS(100); __DPRINTFS_X( "@@@ %d -> %d\n", val, _rxd() );
	}
	//*/
	...
	if(true) {
		// LUTs
		static const int16_t PROGMEM delay_lut   [] = {    0,    1,    5,   10,  50, 100, 500, 1000, 5000 };
		static const int16_t PROGMEM freq_lut_100[] = { 1500, 1400, 1200, 1000, 400, 250,  50,   25,    5 };
		// Allowed baudrate range
		static const uint32_t min_freq =    500U;
		static const uint32_t max_freq = 150000U;
		// Ensure the baudrate is within range
		if(baudrate < min_freq) baudrate = min_freq;
		if(baudrate > max_freq) baudrate = max_freq;
		// Calculate baudrate / 100
		const int32_t baudrate100 = (baudrate + 50U) / 100U;
		...
		for(uint8_t i = 0; i < sizeof(delay_lut) / sizeof(delay_lut[0]) - 1; ++i) {
			...
			const int32_t hfb = pgm_read_word(freq_lut_100 + i    );
			const int32_t lfb = pgm_read_word(freq_lut_100 + i + 1);
			// Check if the baudrate is inside the limit
			if(baudrate100 <= hfb && baudrate100 >= lfb) {
				...
				_xckDelay = ( ldv * (hfb - baudrate100) + hdv * (baudrate100 - lfb) + (rfb / 2U) ) / rfb;
				break;
			}
		}
	}
...
void bbusrt_end(void)
{
	...
	BB_USRT_DDR &= ~( _BV(BB_USRT_RXD_BIT) | _BV(BB_USRT_TXD_BIT) | _BV(BB_USRT_XCK_BIT) );
