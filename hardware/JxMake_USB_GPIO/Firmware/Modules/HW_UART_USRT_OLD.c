...
static void _hwuxrt_disable(void)
{
	// Disable the UART
	UCSR1B &= ~_BV(TXEN1);
	UCSR1B &= ~_BV(RXEN1);
	...
	if(_hwuxrtSyncMode && !_hwuxrtXMegaPDIMode) {
...
#define HWUXRT_DISABLE_TX() do {                             \
		/* Wait until all data has been fully transmitted */ \
...
void hwuxrt_sendByte(uint8_t value)
{
	...
	// Check if the Tx needs to be disabled
...
void hwuxrt_disableTx(void)
{
	...
	// Simply exit if the Tx is not enabled
	if(!_hwuxrtTxEnabled) return;
