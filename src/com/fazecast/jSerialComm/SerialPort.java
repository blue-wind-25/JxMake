 ...
 */
@SuppressWarnings( { "deprecation", "restricted", "unused" } )
		...
		try
		{
			...
			if (!isAndroid)
			{
				...
				for (int attempt = 0; !libraryLoaded && (attempt < 2); ++attempt)
				{
					...
					for (int i = 0; !libraryLoaded && (i < architectures.length); ++i)
					{
/* MODIFIED */			String manualJARLibraryPath = System.getProperty("jSerialComm.jar.library.path", "");
/* MODIFIED */			if (!manualJARLibraryPath.isEmpty()) manualJARLibraryPath += "/";
/* MODIFIED */			InputStream fileContents = SerialPort.class.getResourceAsStream("/" + manualJARLibraryPath + libraryPath + "/" + architectures[i] + "/" + libraryFileName);
//						InputStream fileContents = SerialPort.class.getResourceAsStream("/" + libraryPath + "/" + architectures[i] + "/" + libraryFileName);
	...
	/* MODIFIED */
	static public void removeShutdownHook(Thread hook)
	{
		libraryLock.lock();
		try { shutdownHooks.remove(hook); }
		finally { libraryLock.unlock(); }
	}
	/* MODIFIED */
