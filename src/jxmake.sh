...
DEP_CP=(jansi.jar jSerialComm.jar spellchecker.jar rstaui.jar rsyntaxtextarea.jar autocomplete.jar java-does-usb.jar pty4j.jar annotations.jar slf4j-api.jar kotlin-stdlib.jar jna.jar jna-platform.jar)
...
if [ "$NEEDS_NATIVE_ACCESS" == "true" ]; then
    FLAGS=("--enable-native-access=org.fusesource.jansi,com.fazecast.jSerialComm,net.codecrete.usb,com.sun.jna,com.sun.jna.platform,ALL-UNNAMED" "${FLAGS[@]}")
fi
