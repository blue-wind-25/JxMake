...
- **json_core_inp/out.json** — strict RFC 8259 grammar: quoted keys only, no trailing
  ...
  Covers: top-level colon alignment across fifteen keys of varying width (§1.1),
  including `false`/`null` literals joining the group alongside `true`;
  `"tags"`/`"scores"` as tight atom arrays vs. `"metrics"` going loose because its
  elements are objects (§1.2); `"matrix"` as an array whose elements are
  themselves arrays, going loose at the outer level while each inner `[1, 2]`
  stays tight since its own content is atoms only — the loose/tight decision is
  made independently at each nesting level, not inherited wholesale; `"nested"`
  starting its own independent alignment group at the deeper nesting depth; the
  mandatory space-before-colon even on the single-key `{ "id" : 1 }`/
  `{ "id" : 2 }` objects inside the loose array; `"emptyObj"`/`"emptyArr"` as
  empty-bracket tightness (`{}`, `[]`) sitting in the same top-level group as
  everything else, confirming empty brackets don't get the loose treatment
  nested/complex brackets get; `"price"`/`"factor"` as negative and
  exponent-notation numeric literals, left untouched; `"label"`'s `\u` escape
...
- **json5_combined_inp/out.json5** — JSON5-specific syntax (unquoted keys, trailing
  ...
  alongside a single-quoted key (`'label'`) and a double-quoted key needing
  quotes (`"with-dash"`) in the same alignment group — key quote style is
  preserved per-key, never normalized to one style; trailing commas preserved
  unconditionally, on tight single-line arrays (`tags`) exactly as on loose
  multi-line arrays (`matrix`, `metrics`) and the nested `config` object — "kept
  as written" does not depend on layout; the same colon-alignment/loose-array
  behavior as `json_core` above, now with JSON5 syntax instead of strict JSON,
  including a genuinely nested array (`matrix`) going loose at the outer level
  while inner arrays stay tight; `hex`/`signed`/`leadingDot` exercising JSON5's
  wider numeric grammar (`0xFF`, `+5`, `.5`), left untouched; §1.3's
  line-continuation multi-line strings — `noIndent`'s continuation lines stay
  flush-left and `indented`'s keep their original 4-space lead exactly as
  written, in both cases *despite* the key itself now sitting at the object's
  4-space structural indent — the two must not be conflated. Verified against
  the `json5` npm parser: both `inp`/`out` parse to the identical string value
  (`"First line second line third line"` / `"First line     second line
  third line"`, respectively), confirming reformatting changed only the
  surrounding layout, never the string content.
  </details>
...
- **json5_comments_inp/out.json5** — uncommon `//` and `/* */` placement (JSON5
  ...
  // second leading comment, stacked
  label /* comment before colon */ : "value", // trailing comment too
  }
  ```
  ...
  preceding the single-key `config` group; a standalone comment sitting between
  two elements of `tags`, forcing an otherwise-tight atom array onto multiple
  lines since there's nowhere else for the comment to live; two stacked leading
  `//` comments immediately before `label`, both surviving in order; a `/* */`
  block comment sitting **between a key and its colon**
  (`label /* comment before colon */ :`), the most unusual placement in this
  fixture, left exactly in place per §1's "preserved exactly as written"; a
  trailing `//` comment on the same line as the key/value pair it follows;
  comment text preserved verbatim in position and content — §1's "no
  normalization between the two styles" note means the `//`-vs-`/* */` choice
  itself is never changed — but `normalize-comment-start-case` (a global
  formatter behavior, not something STYLE_DATA_FORMATS.md opts into separately)
  still applies here same as everywhere else: every sentence-fragment comment is
  lowercase-start in `_inp` and capitalized in `_out`.
  </details>
...
- **css_combined_inp/out.css** — property/value colon alignment with mixed group/
  ...
  (`align-items`), then the comment breaking it so `margin`/`--gap`/`padding`/
  `z-index` — all adjacent afterward, with no further blank/comment/boundary
  between them — merge back into one four-member group rather than four isolated
  single-member groups; `--gap` as a CSS custom-property declaration joining an
  ordinary alignment group like any other property; `padding`'s `var()`/`calc()`
  value passed through untouched, since §3.1's colon-alignment rule only governs
  the property/value colon, not the value's own internal content; `z-index: 10
  !important` preserving the flag as ordinary value content; `@media`'s condition
  ...
  two-member group one level deeper; `@supports` as the other at-rule-as-header
  case named alongside `@media` — same recursion, exercised here for the first
  time; `@font-face` as the flat-declaration at-rule case, colon-aligning its own
  two members directly with no nested selector in between; `@keyframes` with
  three percentage selectors (`0%`/`50%`/`100%`), each acting as its own header
  and starting an independent single-member group, each single-line input
  declaration expanding to the project's standard multi-line block shape, with no
  blank lines forced between the stops since `_inp` doesn't have any (blank lines
  around a nested block are preserved-as-written, never mandated — same posture
  `.card`'s own blank lines before `&:hover`/`& .icon` demonstrate below); `.card`
  starting a two-member group, then `&:hover` and `& .icon` each starting a
  fresh, independently-aligned group one level deeper — same header-vs-declaration
  recursion `@media`/`@supports` demonstrate, now via native nesting; `.card`'s
  blank lines before each `&` block carried through unchanged because `_inp`
  already has them, not because the rule requires them (STYLE.md §12's
  optional-grouping posture — nothing here inserts or strips such blank lines
  either way); `.card::before` as an ordinary pseudo-element-selector sibling
  rule with its own independent two-member group.
  </details>
...
- **css_comments_inp/out.css** — uncommon `/* */` placement, plus a
  ...
  /*% JXM_CFMT_ENA */
  padding:8px;
  /* trailing comment, no blank line before close */
  }

  .badge /* comment before brace */ {
  display: inline-block;
  color /* comment before colon */: white;
  }
  ...
  else in its group to align against) — only the comment's first sentence gets
  `normalize-comment-start-case` treatment (`First sentence.`); its second
  sentence is not itself a comment start, so it stays lowercase as written
  (`second sentence.`) — confirmed intentional, not left over from the original
  draft; the `JXM_CFMT_DIS`/`ENA` pair suspending formatting entirely for the
  `border` line in between — preserved byte-for-byte, including its original
  indentation and internal multi-space runs, neither reindented to the rule's
  4-space level nor colon-aligned — with normal formatting (indentation and
  colon alignment) resuming immediately for `padding` after `JXM_CFMT_ENA`; a
  trailing comment immediately before `.alert`'s closing `}`, no blank line
  forced above it; `.badge /* comment before brace */ {` — a block comment
  sitting between a selector and its opening `{`, left in place rather than
  moved or dropped; `color /* comment before colon */: white;` — a block
  comment wedged between a property name and its colon, with the mandatory
  space-before-colon still enforced right after the comment; `.tooltip`'s
  nested `&:hover` block with a comment as the sole line before its
  declarations, checking comment placement inside native-nesting blocks
  specifically, not just flat rule blocks.
  </details>
