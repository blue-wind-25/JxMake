...
This directory has two independent tracked jobs, each with its own
...
supersedes general guesswork for that job. `STATE_KOTLIN.md` is
self-contained and independent of `STATE.md`; do not cross-reference
between the two unless a state file's own text says otherwise.

(The comment-grammar classifier accuracy upgrade, formerly tracked in its
own `STATE_COMMENT_GRAMMAR.md`, shipped and was folded into `STATE.md`'s
"H" section once its checklist completed — see that section and
`RDD_LOG.md`'s `RDD_KEY_94`–`RDD_KEY_98` for its history.)
