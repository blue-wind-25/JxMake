...
>
> §1 and §2 are COMPLETE (see Resolved Design Decisions: "§1 empty parameter list
> (`CppSpecificRule.java`)" and "§2 one-liner scope (`CppSpecificRule.java`)") -- §3 onward remain.

### §1 Empty Parameter Lists
- [x] C always writes `void` explicitly (`void foo(void)`); C++ omits it (`void foo()`) -- see
      Resolved Design Decisions: "§1 empty parameter list (`CppSpecificRule.java`)"

### §2 Function Brace Style (Allman conversion)
- [x] Convert a C/C++ **function definition's** own brace to Allman (own line) when it is
      currently K&R/same-line -- see Resolved Design Decisions: "§2 one-liner scope
      (`CppSpecificRule.java`)"
- [x] One-liner exception -- resolved as brace-placement only (no active collapsing/exploding of
      body line-count); see Resolved Design Decisions: "§2 one-liner scope
      (`CppSpecificRule.java`)"
