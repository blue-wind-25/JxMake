...
- [ ] **Deferred** (see "§15 partial-implementation split"): a `COMMENT_BLOCK` that already spans
      ...
      precondition from the Resolved Design Decision is implemented yet. Needs per-line ` * `
      continuation-marker parsing (to find the first content letter on the first content line and
      the last content character on the last content line without disturbing the markers/
      indentation) before either half can be implemented, plus a chosen mechanic for relocating
      `/*`/`*/` onto their own line on input that doesn't already have that shape -- STYLE.md only
      shows the already-correct target shape, never a "before" example, so the exact relocation
      mechanics are an implementation-time design choice still to be made, not a guess to take now
