...
**In progress dogfood / real-code testing**

(23) Local `../../../src/jxm` (JxMake's own Java source tree, real `.java` files plus PCPP
     `.java.in` templates and `.java.inc` include fragments) — methodology below, not yet run.

**Methodology for candidate (23) — `src/jxm`, mixed plain-Java + PCPP-preprocessed Java:**

1. **Copy, preserving relative directory structure.** `.java.in` files may `#include`
   `.java.inc` files by relative path, so a flattened copy (e.g. `--out DIR` basename-flattening)
   would break those includes or silently collide same-named files across subdirectories. Copy
   the whole `../../../src/jxm` tree into `/tmp` with `cp -r`, preserving every subdirectory
   (e.g. `/tmp/jxm_orig/...` mirroring `src/jxm/...` exactly), not a flattened/filtered copy.
   Check `/tmp` first for an existing copy from a prior session before re-copying (per
   STATE_COMMON.md's no-filesystem-wide-find rule — search only `/tmp`/scratchpad).
2. **Scope:** every `*.java`, `*.java.in`, and `*.java.inc` file under that tree.
3. **Round1 / round2 (idempotency), on all three extensions.** Format round1, format round1's
   own output again as round2, diff must be empty — same as any other candidate. The formatter
   must not damage PCPP preprocessor syntax in `.java.in`/`.java.inc` files (directives, `#include`
   lines, macro invocations) — treat any preprocessor corruption as a compile-breaking-severity
   bug, not cosmetic.
4. **Syntax-check the `.java.in` files by first running them through `pcpp_java`,
   in-place in the copied `/tmp` tree** (same directory as the `.java.in` file, so relative
   `#include`s of `.java.inc` siblings resolve):
   ```bash
   java -jar ../../../3rd_party/tools/pcpp_java/pcpp-java-1.30.jar <in_file>.java.in -o <out_file>.java
   ```
   (adjust the relative path to `3rd_party/tools/pcpp_java/pcpp-java-1.30.jar` for wherever the
   command is actually run from). **Do not run `.java.inc` files through `pcpp_java`** — they are
   include fragments, not standalone compilation units, and are not independently preprocessable.
5. **Then syntax-check with `java_sc`** (build/run instructions already in this file's "Tools/
   compiler used" list, tool (6)) — run it over both the real plain `*.java` files and the
   `*.java`  files generated by step 4 from `.java.in`. **Do not use `javac`** — it will produce
   unrelated dependency/classpath errors on this tree, per explicit instruction; `java_sc` is a
   syntax-only AST checker with no such dependency requirement.
6. **When fixing a bug, reformat/re-`pcpp_java`/re-`java_sc` only the affected file(s)** while
   iterating — do not re-run the full three-extension tree pass after every fix. Only after every
   known bug is fixed, run one final full pass (round1, round2, `pcpp_java`, `java_sc`) over the
   whole `src/jxm` scope to confirm zero regressions and full-tree idempotency.
7. **Fixture registration.** Standard case: add `test/real_code_regressions_N_{inp,out}.java`
   (next available `N`) as usual, per STATE_COMMON.md's fixture convention. **If the bug is
   specifically caused by the presence of the PCPP preprocessor** (i.e. it wouldn't reproduce in
   plain Java with no `.java.in`/`.java.inc` involved — e.g. a directive-adjacency or
   directive-inside-a-clause bug, same general class as RDD_KEY_170's C++ `requires`-clause
   preprocessor bug), prefer appending the minimal repro to the existing
   `test/java_preprocessor_method_inp/out.java` fixture pair instead of creating a new
   `real_code_regressions_N` pair — **but if it's unclear which fixture is the better fit, ask
   the user rather than guessing** (per this task's explicit instruction).
8. Record the outcome here (move this entry to "Finished dogfood / real-code testing" once
   complete, per this file's own "When a test completes" convention below) with a one-line
   summary of bugs found/fixed, verified tool list, and fixture names — same format as every
   other numbered entry above.
