...
| Kotlin | STATE_KOTLIN.md | JetBrains/kotlin | DONE - PARTIAL FIX | D3 still open — root cause confirmed, fix attempt reverted (28 fixture regressions); RDD_KEY_235 confirmed GDR/multipass doesn't resolve it (see STATE_CURLY_GDR.md). 2026-08-11: full 16153-file content-diff pass completed (15583 OK, 570 MISMATCH — all sampled as checker tolerance gaps on already-documented legitimate transforms, no new bug); RDD_KEY_278/279 validated at full-corpus scale |
...
| Bash | STATE_TOOLING.md | ohmyzsh/ohmyzsh | DONE - PARTIAL FIX | 17 *.sh/.bash; found+fixed 3 real bugs (case-arm escaped-paren mismatch, root-mode `\'` mis-tokenizing, nested case...esac non-idempotency — fixtures real_code_regressions_188-189) plus 1 syntax-corruption bug (`>|` noclobber redirect split by pipe-spacing — fixture 190). After fixes: idempotent across all 17. `bash -n` shows 10 pre-existing errors on original and round1 (5 files use zsh-only syntax under .sh/.bash, already invalid bash pre-formatting); one file's pipe-spacing rule also inserts a space inside a zsh extended-glob alternation — documented accepted known gap (dialect detection out of scope) |
