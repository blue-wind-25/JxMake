...
| TSX | STATE_JS_TS.md | lemoncode/react-typescript-samples | DONE - FULL FIX | RDD_KEY_313 (2026-08-20): full corpus round1/round2 idempotency sweep (JSX child-indentation parser follow-up). Found+fixed a real bug — `old_class_components_samples/15 Lazy Loading/.../memberForm.tsx` (and its `16 Custom Middleware/` sibling) has a JSX root tag line with pre-existing tab+space-mixed indentation, which `rewriteJsxChildIndentation` read verbatim as `baseIndent` and baked into every child line as literal tabs, while a separate later pass renormalized only the root line — mismatched parent/child indentation styles, non-idempotent. Fixed via a tab-in-`baseIndent` bail (stops the corruption); full one-pass idempotency for this narrow shape remains an accepted residual gap (converges after 2 passes) — see STATE_JS_TS.md's RDD_KEY_313 entry. No fixture added (would itself fail the suite's idempotency check by design) |
...
| Java | STATE_COMMON.md | recurring self-format pass (src/**, 2026-08-21) | DONE - NO BUG | 99 files; round1/round2 idempotent. 9/99 differed from committed `src/` (recently-touched files) — all reviewed by hand, ordinary cosmetic re-style (if/else collapse, call-wrap width, declaration-alignment column width, trailing-period comment stripping), zero content loss, zero flushed-left/dedented lines. Trial JAR built from round1; `make _test_serial` came back 335/337 — the 2 failures (`test/cpp_comments_inp.cpp`, `test/real_code_regressions_217_inp.java`) confirmed pre-existing GRU-classifier/`gru-sync-weights` drift (byte-identical `--diff` output against a JAR built straight from unmodified committed `src/`), same known drift class as 2026-08-16/2026-08-20 entries. round1b/round2b fixed-point check fully empty on all three comparisons. Adopted; `make clean && make test` 337/337 fully green, `make test-server` passed |
