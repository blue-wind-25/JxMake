...
| Java | STATE_COMMON.md | recurring self-format pass (src/**, 2026-08-26, after D14/KotlinSpecificRule/SwitchRule/gdr cleanup) | DONE - NO BUG | 100 files (`*.java` only), round1/round2 idempotent (`diff -ru` empty). Only 1/100 differed from committed `src/`: `rules/MiscRuleCurly.java` — control-flow-paren spacing normalization (`for(int i...)` → `for( int i...; ++i )`, `if(gapToClose.isEmpty())` → `if( gapToClose.isEmpty() )`) alongside a call-paren un-padding (`gapToClose.put( wsStart, ... )` → `gapToClose.put(wsStart, ...)`) plus a resulting multi-line parameter-list rewrap on `hasTopLevelNewlineInBody`'s signature; `java_content_diff.sh` OK (content preserved), hand-reviewed as ordinary cosmetic re-style, zero content loss. `detect_flushed_left_lines.py` on the full round1 tree: 0 hits. Trial JAR built via bare `javac`+`jar` (JAVA_VERSION=8) with the real `code-formatter-ai-assist-weights.json` copied alongside it (the established bare-trial-JAR workaround, per the 2026-08-21/2026-08-24 rows): `make _test_serial JAR_FILE=/tmp/output.jar` **364/364 forward + idempotency, no GRU-artifact failures at all** (weights present this time, unlike prior sessions' bare-trial-JAR runs). Adopted (1 file copied back over `src/`, spot-checked by eye). `make clean && make test` **364/364 forward + idempotency**, same as the pre-pass baseline — no regression. `tools/gru/*.java` (13 files) compiled cleanly against the freshly rebuilt `target/classes`. No real formatter bug found this pass |
