...
| JSX | STATE_JS_TS.md | taniarascia/react-tutorial | DONE - OPEN Q | Real corpus is 5 `.js` files with embedded JSX, zero real `.jsx` files. As-shipped `.js` extension does NOT trigger the boundary-finding pre-pass (extension-gated by design) — round1 on the real `.js` files produces genuine content corruption/truncation (`js_ts_syntax_check.sh` fails 4/5 files). Same 5 files renamed to `.jsx` (supplementary check, exercising the feature as intended): round1/round2 idempotent, `js_ts_syntax_check.sh` 5/5 clean, manual diff confirms byte-for-byte JSX preservation (checker tool itself mis-flags `.jsx` as MISMATCH — a known checker limitation, not a formatter bug). No template-literal-with-JSX content in this corpus (item 10 untested here). See STATE_JS_TS.md dogfood write-up for detail; open question is whether `.js`-with-JSX should be a documented Known Limitation — researched 2026-08-13, see STATE_JS_TS.md's "2026-08-13 research session — JSX-in-`.js`/`.ts` detection" section (still pending user sign-off, not implemented) |
| JSX | STATE_JS_TS.md | ruanyf/react-demos | NOT STARTED | 2nd dogfood candidate: small but many demo dirs with more complex embedded JSX (conditional rendering, spread props) — stresses Increments 5/6 contexts — deferred, future session |
| JSX | STATE_JS_TS.md | reactstrap/reactstrap | NOT STARTED | 3rd dogfood candidate: larger, plain-JS/JSX (no TS) component library, real production use — deferred, future session |
| TSX | STATE_JS_TS.md | microsoft/TypeScript-React-Starter | DONE | 10 real `.tsx` files (incl. subdirectories, `--preserve-tree` used to avoid basename collisions). Round1/round2 idempotent (0-diff). `js_ts_content_diff.js` batch mode 10/10 OK. `js_ts_syntax_check.sh` 10/10 clean. Diffs are cosmetic-only (brace style, closing-brace comments, interface member `:` alignment). No template-literal-with-JSX content in this corpus (item 10 untested here) |
| TSX | STATE_JS_TS.md | Lemoncode/react-typescript-samples | NOT STARTED | 2nd dogfood candidate: small but many samples with more complex embedded TSX (generics, HOCs) — deferred, future session |
| TSX | STATE_JS_TS.md | excalidraw/excalidraw | NOT STARTED | 3rd dogfood candidate: large, popular, pure-TSX production codebase — deferred, future session |
