...
| Java | STATE_COMMON.md | recurring self-format pass (src/**, 2026-08-21) | DONE - NO BUG | 99 files, round1/round2 idempotent, 9/99 differed (recent touches, ordinary cosmetic re-style), zero content loss, zero flushed-left lines. Trial JAR: `_test_serial` 335/337 — 2 failures confirmed pre-existing GRU-classifier/`gru-sync-weights` drift (same class as 2026-08-16/2026-08-20), round1b/round2b fixed-point empty. Adopted; `make clean && make test` 337/337, `make test-server` passed |
| Java | STATE_COMMON.md | recurring self-format pass (src/**, 2026-08-24) | DONE | 100 files, round1/round2 idempotent, round1b/round2b fixed-point empty. 22/100 differed (ordinary cosmetic re-style: Javadoc trailing-period stripping, declaration-alignment re-width, paren spacing, line-wrap reflow, a pre-existing lambda-arrow-spacing quirk), zero content loss, `detect_flushed_left_lines.py` 0 hits. Bare trial JAR (no `gru-sync-weights`): `_test_serial` 340/347, all 7 failures confirmed the same gru-weights artifact (properly-built JAR passed 347/347 same fixtures). Adopted; `make clean && make test` **347/347 forward + idempotency**, `make test-server` passed |
| Java | STATE_COMMON.md | recurring self-format pass (src/**, 2026-08-25, flag forced off) | DONE | 100 files (`*.java` only), round1/round2 idempotent. 6/100 differed (this session's own rename/static-import/dedup edits; ordinary cosmetic re-style), zero content loss, `detect_flushed_left_lines.py` 0 hits, `java_syntax_check.sh` 100/100 clean. Adopted; `make clean && make test` **363/363 forward + idempotency**; `tools/gru/*.java` (13 files) compiled cleanly against rebuilt classes |
| Java | STATE_COMMON.md | recurring self-format pass (src/**, 2026-08-26, after D14/KotlinSpecificRule/SwitchRule/gdr cleanup) | DONE - NO BUG | 100 files (`*.java` only), round1/round2 idempotent. Only 1/100 differed: `rules/MiscRuleCurly.java` control-flow-paren spacing normalization plus a resulting parameter-list rewrap — content-diff OK, ordinary cosmetic re-style. `detect_flushed_left_lines.py` 0 hits. Bare trial JAR (JAVA_VERSION=8, real weights JSON copied alongside — established workaround): `_test_serial` **364/364, no GRU-artifact failures at all** (weights present, unlike prior bare-trial-JAR runs). Adopted (1 file copied back, spot-checked); `make clean && make test` **364/364**, no regression; `tools/gru/*.java` compiled cleanly. No real formatter bug found this pass |
...
| Java/Python3/Bash | STATE_COMMON.md | dogfood-and-adopt (../../JCS, ../../MDXplorer, ../../../3rd_party/tools/pcpp_java, ../../../3rd_party/tools/colordiff/colordiff.py, 2026-08-21) | DONE - NO CHANGE | 61 files (.java/.py/.sh/.ps1; `.cmd` skipped, no Windows-batch support); round1/round2 idempotent; every file byte-identical (md5-verified sample) to committed original — already at the fixed point from the 2026-08-14 adopt, no copy-back needed |
| Java/XML/Makefile | STATE_COMMON.md | dogfood-and-adopt (../../JCS, ../../MDXplorer, ../../../3rd_party/tools/pcpp_java, ../../../3rd_party/tools/colordiff/colordiff.py, 2026-08-22) | DONE | 61 files scanned (.java/.py/.sh/.ps1/.xml/Makefile; `.cmd` skipped); round1/round2 idempotent; JCS/MDXplorer/colordiff.py unchanged, still at prior fixed point; pcpp_java had 4 files change (Makefile/pom.xml alignment re-width, one BOM char literal made explicit, one ZWNJ bracketed) — content-diff/syntax-check clean, adopted |
| Java/XML/Python3/Makefile | STATE_COMMON.md | dogfood-and-adopt (../../JCS, ../../MDXplorer, ../../../3rd_party/tools/pcpp_java, ../../../3rd_party/tools/colordiff/colordiff.py, 2026-08-24) | DONE - NO CHANGE | 63 files scanned (.java/.py/.sh/.ps1/.xml/Makefile; `.cmd`/`.jar`/`.md`/`.txt` skipped); round1/round2 idempotent; content-diff/syntax-check clean, `detect_flushed_left_lines.py` clean (44 curly-family files); every file byte-identical to committed original across all 4 repos — already at the fixed point from the 2026-08-22 adopt, no copy-back needed |
| Java/XML/Python3/Makefile | STATE_LINE_SPLIT_OP.md | Pass 1 Leg C: dogfood-and-adopt (../../JCS, ../../MDXplorer, ../../../3rd_party/tools/pcpp_java, ../../../3rd_party/tools/colordiff/colordiff.py, 2026-08-25, `line-split-operator-priority` flag off) | DONE - NO CHANGE | 62 files scanned (.java/.py/.xml/Makefile; `.cmd`/`.sh`/`.ps1` present-zero this pass); round1/round2 idempotent; every file byte-identical to committed original across all 4 repos — still at the fixed point from the 2026-08-24 adopt, confirming the new operator-split feature is a true no-op with the flag at its default (off) |
| Java/XML/Python3/Makefile | STATE_C_CPP_JAVA.md | dogfood-and-adopt (../../JCS, ../../MDXplorer, ../../../3rd_party/tools/pcpp_java, ../../../3rd_party/tools/colordiff/colordiff.py, 2026-08-27, mid-word-dot abbreviation/extension masking) | DONE | 63 files scanned (.java/.py/.sh/.ps1/.xml/Makefile; `.cmd`/`.jar`/`.md`/`.txt` skipped); round1/round2 idempotent; syntax checks (java/python/bash) clean; `detect_flushed_left_lines.py` clean (44 curly-family files, no suspicious lines). JCS and colordiff.py unchanged, still at the 2026-08-24 fixed point. MDXplorer (`mdx_server.py`, 4 lines) and pcpp_java (4 files: `LexRule.java`, `LexToken.java`, `PlyLogger.java`, `Production.java`, 1 line each) changed — in every case a previously-blocked trailing-period strip on a single-sentence Javadoc/comment now fires because the newly-landed abbreviation/extension-shaped mid-word-dot masking (`str.format()`, `os.scandir()`, `copy.copy`, `System.err`, `{@code "t_INITIAL_error"}`) no longer counts those as a second "sentence-ending" dot; reviewed line-by-line, confirmed correct/intended, no token or meaning loss. Adopted (copied back) |
| Java/XML/Python3/Makefile | STATE_C_CPP_JAVA.md | dogfood-and-adopt re-run (../../JCS, ../../MDXplorer, ../../../3rd_party/tools/pcpp_java, ../../../3rd_party/tools/colordiff/colordiff.py, 2026-08-27, `isCommentRewritable` `looksCodeLike` fix — see RDD_KEY_355) | DONE | 63 files scanned (.java/.py/.sh/.ps1/.xml/Makefile; `.cmd`/`.jar`/`.md`/`.txt` skipped); round1/round2 idempotent; syntax checks (java) clean on every changed file; `detect_flushed_left_lines.py` clean (44 curly-family files, no suspicious lines). MDXplorer and colordiff.py unchanged, still at the fixed point from the same-day run above. JCS (`CompileServer.java`, 1 line) and pcpp_java (5 files: `CppLexer.java`, `LRGeneratedTable.java` x2, `LRTable.java`, `YaccBuilder.java`, `YaccProduction.java`, 1 line each) changed — in every case a `//` comment containing exactly one space-flanked punctuation character (e.g. `+`, or a variable-name-shaped word like `row`/`g`/`j`) was previously blocked from capitalization and/or period-stripping by `isCommentRewritable` mistaking it for a label/value alignment comment; reviewed line-by-line, confirmed correct/intended, no token or meaning loss. Adopted (copied back) |
