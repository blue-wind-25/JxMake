...
| **C6b-C6z (remaining ~72 files, not yet root-caused this session)** | 72 | Sampled shapes: `Unexpected tokens` (several distinct offsets, likely more than one root cause — not yet clustered further), `Property getter or setter expected` (3 files), `Missing '}'` at EOF (several files, possibly overlapping the C3/C4 brace-drift family), `imports are only allowed in the beginning of file` cascading from `Expecting a top level declaration` (`js/js.parser/.../JavaScriptParserBase.kt`/`JavaScriptLexerBase.kt` — **possible syntax-checker false positive, not a formatter bug**: both files start with a UTF-8 BOM (`﻿`) before the license-header comment in the *original* source, and this exact same failure was already present in the pre-C6a baseline (`/tmp/round1b_syntax.log`), i.e. unrelated to any formatter transformation — worth checking whether `kotlin_syntax_check`/the K2 frontend mishandles a leading BOM before assuming this is a real corruption bug in a future session). Left open per this task's explicit allowance ("if a sub-cluster looks like it needs deep new mechanism work... it's OK to leave it as a documented, still-open sub-cluster") — not individually root-caused this pass; effort was concentrated on C6a (the single largest, cleanly root-caused, ~37% of the 114) rather than spreading thin across several under-investigated shapes. `/tmp/c6_remaining.txt` (72 paths, under `/tmp/round1c/...`) and `/tmp/round1c_syntax.log` retained for a future fixing session. **Baseline check (2026-07-28):** ran `kotlin_syntax_check` against all 72 files' *original, unformatted* source (`/tmp/jb_kotlin_kt/kotlin-master/...`, same relative paths). **70/72 pass clean on the original** — confirms these are genuine formatter-induced corruption, not pre-existing errors in JetBrains/kotlin's own source, so all 70 remain valid, worth-root-causing formatter bugs for a future session. **2/72** (`JavaScriptParserBase.kt`/`JavaScriptLexerBase.kt`, the BOM-adjacent `imports are only allowed...`/`Expecting a top level declaration` cascade) **fail identically on the untouched original** — confirms the earlier suspicion: this is a pre-existing `kotlin_syntax_check`/K2-frontend BOM-handling artifact, not a formatter bug at all. These 2 should be excluded from C6's failure count going forward (true remaining C6b-z scope: 70 files, not 72). |
