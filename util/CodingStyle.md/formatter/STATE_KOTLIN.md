...
- **C4 (multi-line-condition `if` collapse) does not reproduce — root
  ...
  *actual* sole syntax error — a `// when val leaseSessionResult = ...`
  ...
  benefit). **Question for the user:** should C4 be removed from the
  Category-1 table (folded into C5's count) or left as-is with this note,
  and should the ~44-file estimate be re-verified per-file before trusting
  it for any other cluster in this table? Not proceeding with C5's fix
  under this task since C5 was not the assigned bug.
