...
| 3 | Brace style (Allman fn bodies / K&R everything else) | (a) for K&R-enforcement direction, **verified**; (c) for Allman-conversion direction, **reclassified, not yet done** | `BlockStructureRule.qualifiesForKAndR`'s `PAREN_KR_KEYWORDS`/`BARE_KR_KEYWORDS` sets already cover Kotlin's exact same control-flow keyword vocabulary (`if/while/for/switch/catch`, `else/do/try/finally`) — confirmed via harness, K&R already gets correctly enforced onto a same-line-with-`)`-before-K&R-construct brace. But the *other* direction — converting a function body's brace from K&R to Allman when it's currently on the same line as `)` — is not shared logic at all; it's `JavaSpecificRule.enforceMethodDefinitionAllmanBraceStyle`/`CppSpecificRule.enforceFunctionDefinitionAllmanBraceStyle`, both per-language, neither with a Kotlin counterpart. Confirmed via harness: a Kotlin `fun foo() { ... }` written K&R is left completely untouched by every shared pass tried (`enforceKAndRBraceStyle` only ever *adds* K&R, never removes it). Needs a new `KotlinSpecificRule` method mirroring those two, not yet written. |
...
| 3.3 | Secondary constructors (Allman body) | (c), **reclassified, not yet done** | Same gap as §3: no shared/per-language pass converts a same-line `constructor(...) {` to Allman. Confirmed via harness (`class Foo { constructor(x: Int) { ... } }` input stays exactly as written). Whatever new §3 Allman-conversion method gets written should cover both a plain function body and a secondary constructor body in the same pass (both are just "a brace after `)` with no named-construct/control-flow keyword before it," per §3's own original reasoning for what triggers Allman). |
