...
pass (most recently C6i, and now C6f partially/C6h fully); C6b, C6e, C6k
remain fully open, C6f has one further unfixed sub-shape (see its row).
...
| **C6f** — multi-line collapse swallows an embedded `//` line comment, corrupting everything after it on the newly-joined line — **PARTIALLY FIXED, two of at least three distinct root causes closed** (largest sub-cluster, C1-grade severity: silent content loss, not just a crash) | ~20, confirmed in several files, overlapped C6h (now folded in, see C6h's row) and C6k | Not one root cause but several, each a different comment-unaware flattening helper reached via a different code path. **(1) FIXED** — `KotlinSignatureRule.parseKotlinParam`'s column-aligned grid rendering of a function parameter's default value flattened the value's tokens (comment included) via the comment-unaware `renderTokens` helper, corrupting the default expression's trailing tokens, the trailing comma, and any sibling params/return type. Fixed by a new `containsLineComment(slice)` bail (`return null` from `parseKotlinParam`, propagating to `parseKotlinSignature` returning null, which `ScopePipelineCurly` treats as "leave this signature's span completely untouched" — confirmed safe) whenever a param's default-value slice contains a `COMMENT_LINE`. **(2) FIXED** — `BlockStructureRule.tryCollapseBraceless` (the braceless-single-statement-body collapse path, sibling to `tryCollapse`) renders its own `if`/`while`/`for` condition via `renderInline`, exactly like `tryCollapse` does — but unlike `tryCollapse` (which already had a `containsLineComment` guard on its own condition render, see that row a few lines above this one), `tryCollapseBraceless` had no such guard at all, so a comment nested arbitrarily deep inside the condition (e.g. inside a trailing-lambda argument of a call nested within the condition, several call-levels deep — not just a top-level condition comment) reached `renderInline` unguarded and got flattened, silently swallowing the rest of the condition, the closing `)`, and (via the `prefix + " " + body` join) the entire braceless body too. Fixed by adding the exact same `containsLineComment` bail `tryCollapse` already had, to `tryCollapseBraceless` too. **(3) NOT YET FIXED, third shape found, no fix attempted** — an expression-bodied function/property (`= \n // leading comment lines \n realExpr`) whose leading run of comment-only lines gets flattened onto one physical line together with the first token of the real expression that follows; confirmed via a synthetic repro (`private fun f(): T =\n    // comment 1\n//    comment 2\n...\n        buildList { ... }` — all leading comment lines and the first line of `buildList {` fuse into one line) mirroring the real corpus file `native/native.tests/testFixtures/.../AbstractNativeBlackBoxTest.kt`'s `buildJUnitDynamicNodes`. Not yet traced to its exact responsible method/line (candidates not yet ruled out: some pass specific to an `=`-led expression body's own line-joining, distinct from both fixes above and from `enforceCallLineBreaking`/`collapseToOneLine`, which were investigated for the trailing-lambda shape fix (2) folds into but did not end up being the actual site of fix (2) either). Only 1 of the 70-file sample exhibited this exact signature (a run of >1 pure-comment lines immediately preceding an expression body's real content), so it's low-incidence relative to (1) and (2), but not yet ruled out elsewhere in the corpus. | `real_code_regressions_156.kt` (covers fixes (1) and (2) together, one repro each); `make test` 204/204 (no count change — both are corruption fixes to already-covered dogfood-original functionality, not new formatting rules). Verified against the real `JetBrains/kotlin` corpus (`ResolutionTesting.kt` for (1), `KClassMembers.kt` for (2)) via `kotlin_syntax_check`; a 70-file cross-cluster sample went from 22/70 to 40/70 syntax-clean after both fixes (18 newly-clean files, several previously attributed to C6h and C6k — see those rows). Shape (3) is documented but unfixed; its 1 known file remains in the still-failing 30/70. |
...
hardest, **two of at least three root causes now FIXED, one shape still
open** — see its row) > C6b (12 files, cheapest — mirrors C2) > C6e (6 files,
extends C3) > C6d (6 files, mirrors C2 in reverse, **now FIXED**) > C6h
...
unknown until re-triaged — likely partially resolved for free by C6f's
fixes, not yet re-verified) > C6i (1 file, **now FIXED**) > C6j
