...
| RDD_KEY_357 | PowerShell comment normalization (§0) gains its own `NO_CAPITALIZE_TOOLS`, refining RDD_KEY_261's "PowerShell gets plain cap only" (user report: `util/JCS/_port.ps1`'s `# java -version ...` was getting capitalized to `# Java ...`, jarring since `java` is a literal external-command name, not English prose). Deliberately narrower than Bash's Unix-tool-name list — cross-platform CLI tool names only (`java`, `javac`, `git`, `node`, `npm`, `python`, `curl`, `ssh`, `docker`, `kubectl`, `mvn`, `gradle`, `dotnet`, `pip`, `go`, `cargo`), not PowerShell's own cmdlet/keyword surface (still judged too broad to curate, per RDD_KEY_261's original rationale). `make test` 366/366 unchanged; `util/JCS/_port.ps1`'s already-capitalized `Java` manually reverted to `java` since the normalizer only skips capitalizing, it doesn't un-capitalize a prior run's output |
