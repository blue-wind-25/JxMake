...
public final class CssSpecificRule {
    ...
    private void collectTrivia(final Cursor c, final List<String> comments, final boolean[] blankOut) {
        ...
        while (c.cur() != null) {
            final Token t = c.cur();
            if (t.frozen) {
                break; // `/*% JXM_CFMT_DIS */` span -- let the caller capture it raw
            }
    ...
    private List<Item> parseBlockBody(final Cursor c, final boolean topLevel) {
        ...
        while (true) {
            ...
            int peek = c.i;
            while (peek < c.toks.size() && (c.toks.get(peek).type == TokenType.WHITESPACE
                    || c.toks.get(peek).type == TokenType.NEWLINE)) {
                peek++;
            }
            ...
            if (Token.isPunct(t, "}")) {
                if (topLevel) {
                    throw new CssParseException("unexpected '}' at top level");
                }
                c.i++;
                if (!item.leadingComments.isEmpty() || item.blankBefore) {
                    items.add(item);
            ...
            while (true) {
                ...
                if (parenDepth == 0 && colonOffset < 0 && Token.isPunct(tok, ":")) {
            ...
            if (terminator == TERM_BRACE_OPEN || colonOffset < 0) {
                for (int k = preColonComments.size() - 1; k >= 0; k--) {
                    text.insert(preColonCommentPos.get(k), preColonComments.get(k));
                }
            }
    ...
    private void renderItems(final List<Item> items, final int depth, final StringBuilder out) {
        ...
        for (int i = 0; i <= items.size(); i++) {
            ...
            final boolean isAlignableDecl = !atEnd && items.get(i).decl != null && items.get(i).decl.hasColon
                    && items.get(i).decl.midComment == null;
            final boolean breaksBefore = atEnd || !isAlignableDecl
                    || !items.get(i).leadingComments.isEmpty() || items.get(i).blankBefore;
            if (breaksBefore) {
                if (groupStart >= 0 && i > groupStart) {
                    keys.clear();
                    for (int g = groupStart; g < i; g++) {
                        keys.add(items.get(g).decl.prop);
                    }
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            for (final String comment : item.leadingComments) {
                out.append(indent(depth)).append(FormatterSimpleBraced.reindentBlockComment(comment, indent(depth)))
                        .append('\n');
            }
            ...
            if (item.decl == null && item.rule == null) {
                continue; // dangling trailing-comment placeholder
            }
            ...
            if (item.rule != null) {
                ...
                if (item.decl.midComment != null) {
                    out.append(item.decl.prop).append(' ').append(item.decl.midComment).append(" : ")
                            .append(item.decl.value).append(';');
                } else {
                    out.append(item.decl.prop).append(padding[i] != null ? padding[i] : " ")
                            .append(": ").append(item.decl.value).append(';');
                }
            } else {
