...
public class GetterSetterRuleCurly extends GetterSetterRuleCore {
    ...
    public List<List<Member>> groupOneLiners(final List<Token> scopeTokens, final int depth)
    {
        ...
        for( final int[] span : spans ) {
            ...
            if(m != null && !isClassScope && !m.isDefinition) m = null;
            if(m == null) {
            ...
            if( !current.isEmpty() ) {
                ...
                final boolean kindChanged = prev.isDefinition != m.isDefinition || (prev.pureSpecifier == null) != (m.pureSpecifier == null);
                if(kindChanged) {
                    if( current.size() >= 2 ) groups.add(current);
                    current = new ArrayList<>();
                }
    ...
    public List<Member> excludeOutliers(final List<Token> tokens, final List<Member> group)
    {
        ...
        while( remaining.size() >= 2 ) {
            ...
            if(maxWidth > secondWidth * OUTLIER_RATIO) remaining.remove(maxIdx);
            else                                       break;
        } // while
    ...
    public List<String> render(final List<Token> tokens, final List<Member> group)
    {
        ...
        if(isDef) {
            ...
            for( int i = 0; i < group.size(); ++i ) {
                ...
                if( !canSplitParams || typeTexts[i] == null ) {
                    ...
                    paramsCell = padRight(
                        typeTexts[i], maxTypeWidth
                    ) + " " + padRight(
                        nameTexts[i], maxNameWidth
                    );
                }
        ...
        for( int idx = 0; idx < group.size(); ++idx ) {
            ...
            if(lang.isJava) {
                ...
                for(final Token mod : m.modifiers) {
                    final int rank = modifierPriority.priorityOf(mod.text);
                    if(rank >= 0) modCells[rank] = mod.text;
                }
            ...
            if(hasTemplatePrefix) cells.add(
                cellText(tokens, m.templatePrefixFrom, m.templatePrefixTo)
            );
     ...
     */
    protected Member parseOneLinerMember(
    ...
    final List<Token> modifiers = new ArrayList<>();
        ...
        if(returnTypeFrom < 0) return null;
    ...
    final int beforeName = prevSignificant(tokens, nameFrom - 1, returnTypeFrom);
        if( beforeName >= 0 && beforeName >= returnTypeFrom && tokens.get(
            beforeName
        ).type == TokenType.KEYWORD && "operator".equals(
            tokens.get(beforeName).text
        ) ) nameFrom = beforeName;
        else if( beforeName >= 0 && beforeName >= returnTypeFrom && isOp(
            tokens.get(beforeName), "~"
        ) ) nameFrom = beforeName;
        while(nameFrom > returnTypeFrom) {
            final int prevA = prevSignificant(tokens, nameFrom - 1, returnTypeFrom);
            if( prevA < 0 || !isOp( tokens.get(prevA), "::" ) ) break;
            int prevB = prevSignificant(tokens, prevA - 1, returnTypeFrom);
            if(prevB < 0 || prevB < returnTypeFrom) break;
            ...
            if(tb.type != TokenType.IDENTIFIER && tb.type != TokenType.KEYWORD) break;
            nameFrom = prevB;
        } // while
        ...
        for(int k = returnTypeFrom; k < returnTypeTo; ++k) {
            final Token t = tokens.get(k);
            if( t.type == TokenType.KEYWORD && STATEMENT_KEYWORDS.contains(t.text) ) return null;
        }
        ...
        while(true) {
            ...
            if( !isPostParenQualifier(t) ) break;
            if( "override".equals(t.text) ) return null; // Never group override declarations
    ...
    final int    eqSigIdx      = nextSignificant(tokens, afterParen, to);
        ...
        if( isPunct(terminator, "{") ) {
            if(pureSpecifier != null) return null; // = 0 { ... } is not a valid shape
            isDefinition = true;
        ...
        return new Member(
            modifiers, templatePrefixFrom, templatePrefixTo, effectiveReturnTypeFrom, returnTypeTo,
    ...
    {
        if(lang.isJava || t.type != TokenType.KEYWORD) return false;
        switch(t.text) {
    ...
    private boolean hasAccessSpecifier(final List<Token> tokens)
    {
        ...
        for( int i = 0; i < tokens.size(); ++i ) {
            ...
            if( isPunct(t, "{") ) {
                ...
                if( next >= 0 && isOp( tokens.get(next), ":" ) ) return true;
            }
        } // for
