...
public class KotlinGetterSetterRule extends GetterSetterRuleCurly {
     ...
     */
    private Member parseKotlinAccessorMember(
    ...
    )
    {
        ...
        if( valVarIdx < 0 || valVarIdx != pos || tokens.get(
            valVarIdx
        ).type != TokenType.KEYWORD || !( "val".equals(
            tokens.get(valVarIdx).text
        ) || "var".equals(
            tokens.get(valVarIdx).text
        ) ) ) return null;
    ...
    final int    colonIdx = nextSignificant(tokens, scanFrom, declTo);
        ...
        if( accFirstSig < 0 || hasBlankLineRun(tokens, accFrom, accFirstSig) ) return null;
        ...
        for(int k = accFrom; k < accFirstSig; ++k) {
            ...
            if(kt == TokenType.COMMENT_LINE || kt == TokenType.COMMENT_BLOCK) return null;
        }
        if( tokens.get(
            accFirstSig
        ).type != TokenType.KEYWORD || !"get".equals(
            tokens.get(accFirstSig).text
        ) ) return null;
        final int parenOpenIdx = nextSignificant(tokens, accFirstSig + 1, accTo);
        if( parenOpenIdx < 0 || !isPunct( tokens.get(parenOpenIdx), "(" ) ) return null;
        final int parenCloseIdx = nextSignificant(tokens, parenOpenIdx + 1, accTo);
        if( parenCloseIdx < 0 || !isPunct(
            tokens.get(parenCloseIdx), ")"
        ...
        if( hasNewlineBetween(
            tokens, eqIdx + 1, accTo
        ) ) return null; // Multi-line expression body -- not a one-liner

        int lastIdx = accTo - 1;
        while( lastIdx > eqIdx && ( tokens.get(
            lastIdx
        ).type == TokenType.WHITESPACE || tokens.get(
            lastIdx
        ).type == TokenType.NEWLINE ) ) lastIdx--;
        if(lastIdx <= eqIdx) return null; // No expression body
        ...
        if(bodyFrom >= bodyTo) return null; // Empty expression body

        return new Member(
            new ArrayList<Token>(), declLeadFrom, declLeadFrom,
    ...
    private List<int[]> splitKotlinMemberSpans(final List<Token> scopeTokens)
    {
        ...
        while(i < n) {
            ...
            if( !isInsignificant(t) ) sawSignificant = true;
            ++i;
        } // while
        if(start < n) {
     ...
     */
    private Member parseKotlinOneLinerMember(
        final List<Token> tokens,
        final int         from,
        final int         to,
        final int         nestDepth
    )
    {
        ...
        if( hasNewlineBetween(tokens, firstSig, to) ) return null;
        ...
        if( funIdx < 0 || tokens.get(
            funIdx
        ).type != TokenType.KEYWORD || !"fun".equals(
            tokens.get(funIdx).text
        ...
        if(parenCloseIdx < 0) return null;
    ...
    final int    colonIdx           = nextSignificant(tokens, scanFrom, to);
        if( colonIdx >= 0 && isOp( tokens.get(colonIdx), ":" ) ) {
            ...
            if(typeEnd < 0) return null; // No '=' after the return type -- not an expression body
            final int typeTrimEnd = trimTrailingWs(tokens, typeStart, typeEnd);
            if(typeTrimEnd <= typeStart) return null;
        ...
        if( hasBreakableCall(tokens, bodyFrom, bodyTo) ) {
            ...
            if(estimatedWidth > lineLengthLimit) return null;
        }

        return new Member(
            new ArrayList<Token>(), returnTypeFrom, returnTypeFrom,
    ...
    {
        if( !group.isEmpty() && isAccessorMember(
            group.get(0)
        ) ) return renderAccessorGroupRaw(
            tokens, group
        );
    ...
    {
        if( group.size() <= 1 || indentWidth <= 0 ) return render(tokens, group);
        ...
        while(true) {
            ...
            for( int i = 0; i < group.size(); ++i ) {
                if( !excluded[i] ) safeIndices.add(i);
            }
            ...
            if(overflowAt < 0) break; // Fixed point reached -- no remaining row overflows its budget
            excluded[overflowAt] = true;
        } // while

        final List<String> result = new ArrayList<>( java.util.Collections.nCopies(
            group.size(), null
        ) );
              int          i      = 0;
        while( i < group.size() ) {
            if( excluded[i] ) {
                ...
                result.set(
                    i,
                    ( accessorShape ? renderAccessorGroupRaw(tokens, solo) : renderRaw(tokens, solo) ).get(0)
                );
                ++i;
                continue;
            } // if
    ...
    private List<String> renderRaw(final List<Token> tokens, final List<Member> group)
    {
        ...
        for(final Member m : group) {
            ...
            final String callCell = cellText(
                tokens, m.nameFrom, m.nameIdx + 1
            ) + "(" + cellText(
                tokens, m.paramsFrom, m.paramsTo
            ) + ")";
            cells.add(callCell);
            cells.add( m.postParenQualifier.isEmpty() ? "" : ": " + m.postParenQualifier );
            cells.add( "= " + cellText(tokens, m.bodyFrom, m.bodyTo) );
            if(m.trailingComment != null) cells.add(m.trailingComment.text);
            grid.addRow( cells.toArray( new String[0] ) );
        } // for
        final List<String> lines = new ArrayList<>();
        for( final String[] row : grid.flush() ) lines.add( String.join(" ", row) );
