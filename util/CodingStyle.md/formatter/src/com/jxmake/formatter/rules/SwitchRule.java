...
public class SwitchRule {
    ...
    private List<CaseLabel> findCaseLabels(
    ...
    private boolean isNonInline(final List<Token> tokens, final SwitchBlock sw)
    {
        for(final CaseLabel c : sw.cases) {
            if( caseSpansMultipleLines(tokens, c.colonIdx + 1, c.bodyEnd) ) return true;
        }
    ...
    private boolean caseSpansMultipleLines(final List<Token> tokens, final int from, final int to)
    {
        ...
        if(firstSig < 0) return false; // empty/fallthrough body -- trivially fits on one line
    ...
    public String formatNonInlineSwitches(final List<Token> initialTokens)
    {
        ...
        while(true) {
            ...
            if(target == null) return render(
                tokens, Collections.emptyMap(), Collections.emptyMap()
            );
     ...
     */
    private SwitchBlock pickInnermostNeedingWork(
        final List<Token>       tokens,
        final List<SwitchBlock> switches
    )
    {
    ...
    private void applyNonInlineBlankLines(
        final List<Token>          tokens,
        final SwitchBlock          sw,
    ...
    )
    {
        ...
        for(final CaseLabel c : sw.cases) {
            ...
            if(contentEnd < 0) continue; // empty/fallthrough body -- no blank line (preserves the §13 example)
    ...
    private void applyNonInlineCaseIndent(
        final List<Token>          tokens,
        final SwitchBlock          sw,
    ...
    )
    {
        ...
        for(final CaseLabel c : sw.cases) {
            ...
            if( braceClose < 0 || braceClose >= c.bodyEnd || !containsNewline(
                tokens, braceIdx, braceClose
            ) ) continue;
    ...
    private int caseBodyBraceIndex(final List<Token> tokens, final CaseLabel c)
    {
        ...
        while(i < c.bodyEnd) {
            final Token t = tokens.get(i);
            if(t.type == TokenType.NEWLINE) return -1;
            if( !isGapToken(t) ) return isPunct(t, "{") ? i : -1;
            ++i;
        }
    ...
    {
        if( caseIndent.length() > switchIndent.length() && caseIndent.startsWith(
            switchIndent
        ) ) return caseIndent.substring(
            switchIndent.length()
        );
    ...
    private boolean containsNewline(final List<Token> tokens, final int from, final int to)
    {
        for(int j = from; j < to; ++j) {
            if( tokens.get(j).type == TokenType.NEWLINE ) return true;
        }
    ...
    {
        int i = idx;
        while( i > 0 && tokens.get(i - 1).type != TokenType.NEWLINE ) i--;
    ...
    )
    {
        ...
        for(int j = from; j < to; ++j) {
            if( tokens.get(j).type == TokenType.NEWLINE ) {
                final int next = j + 1;
                if( next < to && tokens.get(
                    next
                ).type != TokenType.NEWLINE ) shiftLineIndent(
                    tokens, next, delta, overrides
                );
            } // if
        } // for
    }
    ...
    private void shiftLineAndFollowing(
        final List<Token>          tokens,
        final int                  anchorIdx,
        final int                  to,
        final int                  delta,
    ...
    public String markFallthrough(final List<Token> tokens)
    {
        ...
        for(final SwitchBlock sw : switches) {
            for( int i = 0; i < sw.cases.size() - 1; ++i ) {
                ...
                if( firstSignificantIndex(
                    tokens, from, to
                ) >= 0 ) continue; // Has real content -- not a fallthrough candidate
                if( findFallthroughMarker(
                    tokens, from, to
                ) != -1 ) continue; // Already marked, or an unrelated comment is present -- leave it alone
                if( tokens.get(c.colonIdx).frozen ) continue;
    ...
    private CaseRow classify(final List<Token> tokens, final CaseLabel c, final boolean isLast)
    {
        ...
        if(markerIdx == -2) return null; // An unrelated comment is present -- don't guess, leave the switch alone

        final int firstSig = firstSignificantIndex(tokens, from, to);
        if(firstSig < 0) {
            final int lastIdx = markerIdx >= 0 ? markerIdx : c.colonIdx;
            return new CaseRow(
                c.kwIdx, lastIdx, label, false, false, null, null, null, false, !isLast
            );
        }
        ...
        if( isKeyword( tokens.get(firstSig), "break" ) ) {
            ...
            return new CaseRow(
                c.kwIdx, semi, label, true, false, null, null, "break", false, false
            );
        } // if
        ...
        if(afterSemi < 0) return new CaseRow(
            c.kwIdx, semi, label, true, callShaped, name, args, plain, false, false
        );
        if( isKeyword( tokens.get(afterSemi), "break" ) ) {
            ...
            return new CaseRow(
                c.kwIdx, semi2, label, true, callShaped, name, args, plain, true, false
            );
        } // if
    ...
    private int findFallthroughMarker(final List<Token> tokens, final int from, final int to)
    {
        ...
        for(int i = from; i < to; ++i) {
            final Token t = tokens.get(i);
            if( isComment(t) ) {
                if( found >= 0 || !t.text.contains("FALL-THROUGH") ) return -2;
                found = i;
            }
        } // for
     ...
     */
    private void applyInlineAlignment(
        final List<Token>          tokens,
    ...
    )
    {
        ...
        for( int i = 0; i < rows.size(); ++i ) {
            ...
            if(!row.hasContent && !row.hasBreak) {
                ...
                final String terminator = (row.hasContent ? ";" : "") + ( row.hasBreak ? (row.hasContent ? " break;" : "break;") : "" );
                outerGrid.addRow(
                    new String[] {label, row.hasContent ? content[i] : "", terminator}
                );
            }
        } // for
        ...
        for( int i = 0; i < rows.size(); ++i ) {
            ...
            if( indentLen + rowText[i].length() > lineLengthLimit ) return;
        } // for
        ...
        for( int i = 0; i < rows.size(); ++i ) {
            ...
            for(int k = row.kwIdx + 1; k <= row.lastIdx; ++k) overrides.put(k, "");
        }
    }
