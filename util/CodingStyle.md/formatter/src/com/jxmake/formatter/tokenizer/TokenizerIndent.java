...
public class TokenizerIndent extends TokenizerCore {
    ...
    public List<Token> tokenize(final String source)
    {
        ...
        while(pos < length) dispatchToken(tokens);
    ...
    final int     n           = tokens.size();
        ...
        while(i < n) {
            ...
            if(t.type == TokenType.NEWLINE) {
                ...
                if( !insideBrackets && !isBackslashContinuation(out) ) atLineStart = true;
            }
        } // while
        ...
        while( indentStack.peek() > 0 ) {
            indentStack.pop();
            out.add(
                new Token( TokenType.DEDENT, String.valueOf( indentStack.peek() ), 0, 0, null )
            );
        }
        ...
        if(c == '\r' || c == '\n') {
            ...
            if( idTok.type == TokenType.IDENTIFIER && followedByQuote
                ...
                if( idTok.text.toLowerCase(
                    ...
                    emitFString(q, triple)
                );
                else if(triple) out.add( emitTripleQuotedString(q) );
                else out.add( emitSimpleString(q) );
            } // if
            else {
                out.add(idTok);
            }
    ...
    private List<Token> emitFString(final char quote, final boolean triple)
    {
        ...
        while(pos < length) {
            ...
            if(atClosingQuote) {
                if(pos > literalStart) out.add(
                    new Token( TokenType.FSTRING_MIDDLE, source.substring(literalStart, pos), braceDepth, parenDepth, null )
                );
    ...
    private Token emitFStringFormatSpec()
    {
        ...
        return new Token(
            TokenType.FSTRING_FORMAT_SPEC, source.substring(start, pos), braceDepth,
            parenDepth, null
        );
    }
    ...
    private Token emitWalrus()
    {
        ...
        return new Token(
            TokenType.OP, source.substring(start, pos), braceDepth, parenDepth,
            null
        );
    }
