...
void audio_start(void) {
    if (g_state == AUDIO_STATE_IDLE || g_state == AUDIO_STATE_PAUSED) {
        g_state = AUDIO_STATE_RUNNING;
    }
}
...
static bool audio_validate(const AudioConfig* cfg) {
    ...
    if (cfg->channels == 0 || cfg->channels > AUDIO_MAX_CHANNELS) {
        return false;
    }
    switch (cfg->format) {
...
void audio_debug(const AudioConfig* cfg /* config */, char* out /* output */, size_t len /* buffer size */) {
    if /* sanity check */ (cfg == NULL || out == NULL) {
        return;
    }
    // Format: "ch=%u rate=%u"
