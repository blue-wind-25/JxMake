    ...
    {
        if( isParameter(node) ) return emptyInferenceResult;
        ...
        if( isConstAssertion(node) ) return relativeType(node.expression);
        if( isArrayLiteralExpression(node) ) {
            const variableDecl = findAncestor(node, isVariableDeclaration);
            const partName     = variableDecl && isIdentifier(
                variableDecl.name
            ) ? variableDecl.name.text : undefined;
        ...
        if( isVariableDeclaration(node) && node.initializer ) return relativeType(node.initializer);
    ...
    function typeToTypeNode(
        type: Type, enclosingDeclaration: Node, flags = NodeBuilderFlags.None
    ): TypeNode | undefined
    {
        ...
        if(!minimizedTypeNode) return undefined;
        const result = typeNodeToAutoImportableTypeNode(
            minimizedTypeNode, importAdder, scriptTarget
        );
        return isTruncated ? factory.createKeywordTypeNode(SyntaxKind.AnyKeyword): result;
    }

    function typePredicateToTypeNode(
        typePredicate: TypePredicate, enclosingDeclaration: Node, flags = NodeBuilderFlags.None
    ): TypeNode | undefined
    {
        let isTruncated = false;
    ...
    function addTypeToVariableLike(
        decl: ParameterDeclaration | VariableDeclaration | PropertyDeclaration
    ): DiagnosticOrDiagnosticAndArguments | undefined
    {
        const { typeNode } = inferType(decl);
        if(typeNode) {
            if(decl.type) changeTracker.replaceNode( getSourceFileOfNode(decl), decl.type, typeNode );
            else changeTracker.tryInsertTypeAnnotation( getSourceFileOfNode(decl), decl, typeNode );
    ...
    function typeToStringForDiag(node: Node) {
        ...
        if(result.length > defaultMaximumTruncationLength) return result.substring(
            0, defaultMaximumTruncationLength - "...".length
        ) + "...";
