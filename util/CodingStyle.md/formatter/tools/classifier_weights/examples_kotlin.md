...
| 68 | `this receiver was captured accidentally when the lambda moved outside the class body` | no | no | no | no | NO | "this" naming the actual receiver under retrospective discussion, code reference despite zero mechanical feature. |
| 69 | `this scope function here was chosen specifically to avoid an extra null check` | no | no | no | no | NO | "this" naming the actual `this`-scoped construct being described in flowing prose, code reference despite zero mechanical feature. |
| 70 | `this is honestly the cleanest workaround anyone on the team has found so far` | no | no | no | no | YES | "this" as an ordinary English demonstrative pronoun opening a sentence, prose. |
| 71 | `this file has grown unwieldy and should probably be split before the next release` | no | no | no | no | YES | "this" as an ordinary English demonstrative pronoun, prose about the file itself, not a code construct. |
| 72 | `object here was made a singleton specifically to avoid re-parsing the config` | no | no | no | no | NO | "object" naming the actual singleton declaration under retrospective discussion, code reference despite zero mechanical feature. |
