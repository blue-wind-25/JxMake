...
public final class GruSoftmaxSelfTest {
    ...
    private static void checkSoftmaxSumsToOne(double[] logits) {
        ...
        if (Math.abs(sum - 1.0) > EPSILON) {
            fail("softmax(" + java.util.Arrays.toString(logits) + ") sums to " + sum + ", expected 1.0");
        }
    }
    ...
    private static void checkSoftmaxNumericallyStableOnLargeLogits() {
        ...
        for (double v : p) {
            if (Double.isNaN(v) || Double.isInfinite(v)) {
                fail("softmax of large equal logits produced NaN/Infinity: " + java.util.Arrays.toString(p));
                return;
            }
        }
    }
    ...
    private static void checkSoftmaxHighestLogitHighestProbability() {
        ...
        if (!(p[1] > p[0] && p[1] > p[2])) {
            fail("softmax did not rank highest logit's probability highest: "
    ...
    private static void checkSoftmaxEmpty() {
        ...
        if (p.length != 0) {
            fail("softmax of empty logits should return empty array, got "
                    + java.util.Arrays.toString(p));
        }
    }
    ...
    private static void checkDecideReturnsClassAboveThreshold() {
        ...
        if (d != CommentDecision.NO) {
            fail("decide() expected NO for [0.1, 0.8, 0.1], got " + d);
        }
    }
    ...
    private static void checkDecideAbstainsBelowThreshold() {
        ...
        if (d != CommentDecision.ABSTAIN) {
            fail("decide() expected ABSTAIN for a sub-threshold top class, got " + d);
        }
    }
    ...
    private static void checkDecideAbstainsExactlyAtThreshold() {
        ...
        if (d != CommentDecision.ABSTAIN) {
            fail("decide() expected ABSTAIN when top class exactly equals threshold, got " + d);
        }
    }
