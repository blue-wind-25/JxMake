...
public final class GruTokenizerSelfTest {
    ...
    private static void checkTokenize(String input, List<String> expected) {
        ...
        if (!actual.equals(expected)) {
            fail("tokenize(" + quote(input) + ") = " + actual + ", expected " + expected);
        }
    }
    ...
    private static void checkHashDeterministic(String token) {
        ...
        if (a != b) {
            fail("hashBucket(" + quote(token) + ") not deterministic: " + a + " vs " + b);
        }
    }

    private static void checkHashInRange(String token) {
        int h = GruClassifier.hashBucket(token);
        if (h < 0 || h >= GruClassifier.HASH_BUCKETS) {
            fail("hashBucket(" + quote(token) + ") = " + h + " out of range [0, "
                    + GruClassifier.HASH_BUCKETS + ")");
        }
    }

    private static void checkHashDistinct(String a, String b) {
        if (GruClassifier.hashBucket(a) == GruClassifier.hashBucket(b)) {
            System.out.println("GruTokenizerSelfTest: note -- hashBucket(" + quote(a) + ") collides "
                    + "with hashBucket(" + quote(b) + ") (possible for a 1024-bucket hash, not a failure)");
        }
    }
