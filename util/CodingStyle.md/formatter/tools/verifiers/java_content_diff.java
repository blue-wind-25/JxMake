...
public class java_content_diff {
    ...
    static CompilationUnitTree parse(String source) throws IOException {
        ...
        if (compiler == null) {
            throw new IllegalStateException("No system compiler found (need a JDK, not a JRE)");
        }
    ...
    static List<String> importMultiset(CompilationUnitTree cu) {
        ...
        for (ImportTree imp : cu.getImports()) {
            out.add((imp.isStatic() ? "static " : "") + imp.getQualifiedIdentifier().toString());
        }
        out.sort(null);
        return out;
    }
    ...
    static List<String> topLevelDecls(CompilationUnitTree cu) {
        ...
        for (Tree t : cu.getTypeDecls()) {
            out.add(normalizeWhitespace(t.toString()));
        }
        return out;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!origPkg.equals(fmtPkg)) {
            mismatches.add("package declaration changed: '" + origPkg + "' -> '" + fmtPkg + "'");
        }

        // Imports: multiset (legitimately reordered/sorted by java-import-order).
        mismatches.addAll(diffMultisets("imports", importMultiset(origCu), importMultiset(fmtCu)));
